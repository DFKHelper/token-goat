/** A minimal Model Context Protocol server, in-house. This replaces `@modelcontextprotocol/sdk` on the shipping path. The SDK is excellent and Anthropic-maintained, but it is built to be every MCP participant at once -- client and server, stdio and HTTP and SSE, OAuth, resources, prompts, completions, sampling, elicitation, tasks -- and it charges every install for all of it. It brings 91 packages, among them *two* HTTP frameworks (`express` and `hono`), `cors`, `body-parser`, `express-rate-limit`, `jose`, `pkce-challenge`, `qs` and `path-to-regexp`. Token-goat runs one stdio server that registers tools and answers `tools/call`. It reaches none of the HTTP surface, so none of those packages can ever execute here; they are pure install weight and pure supply-chain surface. What token-goat actually used of the SDK is small enough to state in full: `new McpServer(info)`, `registerTool(name, {description, inputSchema}, handler)`, `connect(transport)`, `close()`, and an `onclose` callback. That is what this file implements, plus the four protocol methods a tools-only server has to answer (`initialize`, `ping`, `tools/list`, `tools/call`) and the notifications it has to tolerate. **The SDK stays a devDependency, and that is the point.** Every MCP test in this repository drives this server through the SDK's own `Client` over its own `InMemoryTransport`. So the reference implementation -- schema validation, JSON-RPC framing, capability assertions, protocol-version negotiation and all -- is the thing checking our wire format on every run. A hand-written fake of the client would prove nothing; the real one proves interoperability. **Deliberately not implemented**, because token-goat registers none of them and a stub that answers a method it cannot serve is worse than a clean `Method not found`: resources, prompts, completions, logging, sampling, elicitation, roots, tasks, progress notifications, request cancellation, and every transport other than stdio. Capabilities advertise `tools` only, so a conforming client never asks -- the SDK's own client refuses to send `resources/list` to a server that did not declare `resources`. Wire-format decisions worth stating, because each was read off the reference implementation rather than guessed: - `initialize` echoes the client's requested protocol version when we recognise it, and falls back to our own latest when we do not. That is the spec's negotiation rule, and it is what keeps an older client working instead of being told a version it will reject. - `inputSchema` is JSON Schema generated by `z.toJSONSchema(..., {io: 'input'})`, sent without a `$schema` field. The MCP spec reads a schema without one as 2020-12, the generated schemas mean the same under 2020-12 and draft-07, and the field was repeated in every tool of every `tools/list`, which a client puts in the model's context each turn. The SDK still sends draft-07's identifier; that is the one place this server's `tools/list` differs from it. - A tool handler that throws becomes `{isError: true}` with the message as text, not a JSON-RPC error. That is what the SDK does, and it matters: a JSON-RPC error is a protocol failure the client raises, while `isError` is a tool result the model can read and react to. - Argument validation failures take the same path, so a bad argument reaches the model as a readable message rather than killing the call. */
...
function buildJsonSchema(shape: ToolInputShape | undefined): Record<string, unknown> {
  ...
  const { $schema: _dialect, ...schema } = z.toJSONSchema(z.object(shape), { io: 'input' }) as Record<string, unknown>
  return schema
}
