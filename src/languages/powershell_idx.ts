...
// Blanks the contents of single-line PowerShell string literals so a `{`/`}` inside a string value is never counted as real code structure. PowerShell strings do NOT use backslash as an escape character (unlike `stripStringLiterals` in common.ts, which assumes C-like backslash escaping) - a backslash in a PowerShell string is always a literal character, most commonly seen in Windows path literals like `"C:\Temp\"`. Treating it as an escape (as the shared `stripStringLiterals` does) misreads a trailing backslash immediately before the closing quote as an escaped quote, leaving the string "open" past its real end and swallowing the rest of the line - including any brace characters - as phantom string content, desyncing `braceDepth` for every line after. PowerShell's real escaping rules are: a doubled quote of the same kind (`""` inside a double-quoted string, `''` inside a single-quoted string) is a literal quote, and a backtick immediately inside a double-quoted string escapes the following character (backtick escaping does not apply inside single-quoted strings).
...
// Finds the index of the first unquoted occurrence of `needle` in `text`, tracking single- and double-quoted string state as it scans left to right. Used so that comment markers (`<#`, `#`) that merely appear inside a string literal aren't mistaken for real comment syntax. Regression: this used to toggle inDouble on every literal `"`, with no backtick-escape awareness -- unlike stripPowershellStringLiterals below, which correctly treats a backtick immediately before a `"` inside a double-quoted string as PowerShell's real escape sequence. A backtick-escaped quote (`` `" ``) was misread as the string's real closing quote, so a `#` appearing later on the same (still logically-open) string got treated as a real comment marker and truncated the line -- silently dropping any code, including a closing brace, past that point. Skipping the escaped character here keeps this function's quote tracking in sync with stripPowershellStringLiterals's.
...
export function extractPowershell(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Mask multi-line PowerShell here-string spans (@"..."@ / @'...'@) first, state carried across lines, so braces (and comment-looking sequences) inside one of those can never desync braceDepth or the <# #> block-comment scanner below. Skipped on lines that start already inside a block comment (mlState null) to avoid misreading comment prose that happens to contain opener-shaped text.
    ...
    // Handle <# ... #> block comments. A `<#` only opens a real comment when it isn't sitting inside a quoted string literal - PowerShell strings can legitimately contain that two-character sequence (e.g. "the <# marker").
    let line = rawLine
    let openedBlockCommentThisLine = false
    if (!inBlockComment) {
      const openIdx = findUnquoted(rawLine, '<#')
      if (openIdx !== -1) {
        // A `<#` only opens a real block comment if no unquoted `#` line-comment marker appears earlier on the line. Otherwise the `<#` is just text sitting inside an ordinary `# ...` line comment (e.g. `# See <# for syntax details`), and treating it as an opener would leave inBlockComment stuck true for the rest of the file since no matching #> ever follows. Fall through to the line-comment stripping below instead.
        const hashIdx = findUnquoted(rawLine, '#')
        const isRealOpener = hashIdx === -1 || hashIdx >= openIdx
        if (isRealOpener) {
          const closeIdx = rawLine.indexOf('#>', openIdx + 2)
          if (closeIdx !== -1) {
            // Opens and closes on this same line: blank out the comment span (keeping column positions stable) and keep processing the rest of the line as normal code instead of short-circuiting the whole line.
            line = rawLine.slice(0, openIdx) + ' '.repeat(closeIdx + 2 - openIdx) + rawLine.slice(closeIdx + 2)
          } else {
            // The comment opens here but doesn't close on this line. Any code before the opener (e.g. `function Foo {`) is still real code and must not be discarded - only the swallow-check below (for lines fully INSIDE an already-open comment) should be skipped this pass.
            inBlockComment = true
    ...
    if (inBlockComment && !openedBlockCommentThisLine) {
      ...
      // The comment closes on this line. Slice off everything up to and including `#>` so only the real trailing code (if any) is brace-counted and processed as normal code below, instead of `continue`-ing past it.
      line = rawLine.slice(closeMarkerIdx + 2)
    }

    // Strip an unquoted `#` onward before anything else, so comment text (e.g. `# TODO: handle { edge case`) never desyncs the brace counter or gets mistaken for a declaration.
    ...
    if (currentClass !== null) {
      ...
      if (depthInClass === 1) {
        // Match constructor or method: [returntype] methodName([params]) or static [type] Name(); negative-lookahead rejects PS control keywords as defense in depth The return-type bracket group allows one level of nesting (`[^[\]]|\[[^[\]]*\]`) because PowerShell class methods legally return generic types like `[List[string]]` - a plain `[^\]]*` stops at the first `]` (the inner one), leaving the outer `]` dangling and silently dropping the whole method declaration since the name-capture group can't match `]`.
    ...
    // Apply brace delta BEFORE scope pop check (critical ordering). Brace-count on a string-stripped copy of the line so a literal brace inside a string literal is never counted as real nesting.
