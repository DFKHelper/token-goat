...
/** A resource-table row whose status column reads as anything other than healthy, which is the row the command was almost certainly run to find. Deliberately drawn from the state vocabularies the callers of {@link truncateTableRows} actually print (kubectl pod/node phases and container waiting reasons, AWS resource and instance states, and helm 3's release statuses) rather than from {@link ERROR_SIGNAL_RE}, whose `failed`/`error` substring matching hits a resource merely *named* `error-handler` and would have promoted a healthy row on its name alone. Word-bounded for the same reason. The trailing lowercase entries are helm's: helm 3's release.Status values are lowercase strings that `helm list` prints verbatim, so the capitalised kubectl and CloudFormation entries above matched none of them. They are spelled out one by one rather than by making the whole pattern case-insensitive, because `/i` would additionally promote a kubectl or aws row on a lowercase `error`, `pending` or `terminating` anywhere in a name, namespace or image tag, and would still reach none of helm's statuses that the capitalised entries do not already spell, since the overlap is exactly `failed` and `unknown`. */
export const TABLE_ROW_ANOMALY_RE =
  /\b(?:CrashLoopBackOff|ImagePullBackOff|ErrImagePull|CreateContainerError|CreateContainerConfigError|InvalidImageName|RunContainerError|OOMKilled|Evicted|Terminating|ContainerCreating|PodInitializing|NotReady|SchedulingDisabled|Unschedulable|Pending|Failed|Error|Unknown|Unhealthy|DEGRADED|UNAVAILABLE|STOPPED|STOPPING|TERMINATED|TERMINATING|FAILED|ROLLBACK_COMPLETE|ROLLBACK_FAILED|CREATE_FAILED|UPDATE_FAILED|DELETE_FAILED|failed|unknown|pending-install|pending-upgrade|pending-rollback|uninstalling)\b/
