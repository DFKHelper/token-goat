...
//
// Pytest output is highly structured, so this filter does NOT fit the Node
// test-runner family in `families.ts`: it strips pytest-xdist `[gwN]` worker
// prefixes, collapses pytest-cov coverage tables to their TOTAL line, trims the
// `slowest N durations` section to the first five entries, and deduplicates the
// warnings-summary section — none of which the jest/vitest family models.
//
// Compression model (verbatim from the Python docstring):
//   * Keep   — FAILURES / ERRORS blocks, short-test-summary, warnings summary,
//              the final `= N failed, M passed in Xs =` tally, and the
//              `collected N items` line (first three).
//   * Drop   — the dots/percent progress line, constant banner lines
//              (platform/cachedir/rootdir/plugins/configfile/bringing-up/
//              cacheprovider), `collecting …` preamble, the constant
//              `= test session starts =` header, and every PASSED line (kept as
//              a count, in both default and verbose mode).
//   * xdist  — strip the `[gw0]` worker prefix from every line.
//   * cov    — collapse per-file coverage rows to a single TOTAL line.
//   * slow   — keep the section header + first five entries; collapse the rest.
//   * warn   — dedupe repeated warning messages; drop `-- Docs:` footers.
  ...
  override compress(stdout: string, stderr: string, _exitCode: number, _argv: string[]): string {
    ...
    for (let line of lines) {
      ...
      if (HEADER_RE.test(line)) {
        ...
        inCovTable = false
        // Drop the constant `= test session starts =` header — no signal. All
        // other section headers are preserved verbatim.
      ...
      if (inWarningsSection) {
        ...
        // Everything else in the warnings section is kept verbatim (low-volume
        // context that helps locate the issue) by falling through.
      }
      ...
      // PASSED entries (status-first): count, don't keep — unless inside a
      // failure/error block (captured output may legitimately contain them).
