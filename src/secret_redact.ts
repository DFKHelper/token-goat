...
export function compileCustomPatterns(sources: readonly string[]): {
  ...
  // Blank entries are dropped before the cap rather than after it. The environment variable that carries these is split on line breaks, so a list written with a blank line between groups, or with a trailing newline, arrives with empty entries in it -- and counting those against the cap would silently discard real patterns at the end of the list while reporting nothing.
