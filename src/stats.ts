...
import { creditDivisor } from './token_estimate.js'
...
export function savedTokensFromBytes(bytes: number, cls: ContentClass = 'text'): number {
  return Math.round(Math.max(0, bytes) / creditDivisor(cls))
}
...
const COMMAND_KINDS: Record<string, Set<string>> = {
  ...
  'xlsx-columns': new Set(['xlsx_columns']),
  ...
  'docx-tables': new Set(['docx_tables']),
  ...
  'sqlite-tables': new Set(['sqlite_tables']),
