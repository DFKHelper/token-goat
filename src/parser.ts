/** Tree-sitter source indexer (Layer 7). Parses a source file into {@link SymbolEntry} / {@link RefEntry} rows and upserts them into the SQLite index that `index_reader.ts` and the CLI read back. Tree-sitter grammars (TypeScript / JavaScript / Python) are optional native dependencies: when a grammar fails to load — a build without the native binding, or an unsupported language — extraction degrades to a regex pass that still recovers top-level functions and classes. The Python port (`parser.py`) keeps a richer model (imports/exports, sections, per-file SHA gating). This TS port targets the symbol/ref subset that the read commands surface, matching the simplified `db.ts` schema whose `symbols`/`refs` rows are keyed by absolute `file_path`. */
...
/** Ref names that are never hand-written user code, so a ref bearing one carries no retrieval value under any query and is dropped before it reaches the `refs` table (see the insert loop in {@link writeParseResult}). This is deliberately a very small, unambiguous set, not a general "assertion noise" stoplist -- see the S3 finding this closes for the fuller reasoning: `__name` is esbuild's own compiler-injected helper (it rewrites `function foo(){}` into `foo = __name(function(){...}, "foo")` for `Function.prototype.name` support in its bundle output). Confirmed against a real indexed corpus: every `__name` ref traced back to a `*.snapshot/asset.*.bundle/index.js` file -- a vendored, pre-bundled build artifact, not source a developer wrote. No language grammar this indexer parses ever produces `__name` as a user-authored call site, so excluding it carries no risk of dropping a real caller. Deliberately NOT included here: generic test-framework globals (`expect`, `it`, `describe`, `toBe`, ...), even though they dominate refs-table row count in a JS/TS-heavy index. A project can legitimately define its own same-named export (a custom `describe`/`it`-shaped DSL is not far-fetched), and silently dropping those refs at index time would make `callers describe` permanently return zero for such a project with no error -- the "silently-emptied enumeration passes forever" failure class this codebase has shipped and fixed before. That risk, plus the relatively narrow benefit (each of those names is opt-in: nobody queries `callers expect` by accident), is why S3 did not add a stoplist for them. */
...
// `.h` is inherently ambiguous between C and C++ (unlike `.hpp`, which is unambiguous cpp) -- the same extension is used for both languages. None of `class`, `namespace`, `template<`, `::`, or an access-specifier label (`public:`/`private:`/`protected:`) are valid C syntax, so any match is a strong, low-false-positive signal that a `.h` file is genuinely a C++ header. Used to route such files to the cpp grammar instead of the c grammar -- parsing C++-only syntax with tree-sitter-c still "succeeds" (error recovery doesn't throw) but produces ERROR nodes around it, silently dropping or mis-scoping symbols/refs, exactly the same failure mode `useTsx` below guards against.
const CPP_HEADER_SNIFF_RE = /\bclass\s+\w|\bnamespace\s+\w|\btemplate\s*<|::\s*\w|\b(?:public|private|protected)\s*:/

/** Hard ceiling on the source text stored in `symbols.body`, in characters. `symbols.body` is written once per symbol and mirrored again into the `symbols_fts` full-text index, so an extractor that emits an oversized body costs roughly double on disk and is tokenized by FTS on every reindex. Any extractor bug that makes body size scale with *file* size rather than with *symbol* size therefore inflates the index quadratically, which is what {@link extractJsonSymbols} used to do for minified JSON (see the comment there). A DB bloated that way makes a reindex of the affected file long enough to hold SQLite's single writer lock past db.ts's 15s `busy_timeout`, which surfaces to a concurrent writer (worker daemon vs. CLI/hook) as "database is locked", and stalls `token-goat index` while FTS re-tokenizes gigabytes of duplicated text. This cap is enforced at the single write path ({@link writeParseResult}) rather than in each extractor, so it bounds every current and future language extractor, not just the one that regressed. 128 KB is far above any realistic single function/class body (~32k tokens) while still bounding the pathological case. */
export const MAX_SYMBOL_BODY_CHARS = SYMBOL_BODY_CHAR_CAP

/** Bound what gets *stored* for a symbol body, without losing what gets *read*. An over-cap body is stored as the empty string rather than as a truncated copy. That distinction is the whole point: read_commands.ts's `resolveBody` — the shared accessor behind `read`, `symbol`, `brief`, and frame resolution — already re-slices an empty body from the source file using the symbol's line range, so an elided body is served back complete and correct from disk. Storing a *truncated* body instead would defeat that fallback and make every one of those commands silently return partial source while presenting it as the full symbol, with `line_end` still advertising the complete range. The cost of eliding is confined to what genuinely needs the text resident in the DB: FTS body matching for that one oversized symbol. Losing full-text hits on a >128 KB body is a fair trade against unbounded index growth, and against `read` lying about what it returned. */
export function boundSymbolBody(body: string): string {
  return body.length > MAX_SYMBOL_BODY_CHARS ? '' : body
}

/** Ceiling on a stored `symbols.docstring`. Smaller than the body cap because a docstring is a summary: past a few KB it has stopped being one, and nothing displays it in full. */
...
/** Bound a stored docstring, **truncating** rather than eliding it. This is deliberately the opposite of {@link boundSymbolBody}, for a reason specific to the column. An over-cap *body* is stored empty because read_commands.ts's resolveBody can rebuild it exactly from `[line_start, line_end]` -- eliding costs nothing. No such range is recorded for a docstring, so eliding one destroys it: `outline`'s documented/undocumented flag would flip to "undocumented" for the most heavily documented symbols in a file, which is worse than a visibly-cut docstring. Nothing treats this column as complete source the way `read` treats `body` -- consumers display its first line (read_commands.ts), test it for emptiness, or split it into words (graph_commands.ts) -- so a marked truncation is honest and lossless enough. Python's extractor (`pythonDocstring`) and {@link precedingDocComment}'s callers (TS/JS, Rust, Go, Ruby, Java, C/C++, and the regex fallback) are the only ones that populate `docstring` -- every other extractor still sets `''`. The column is derived from a *region near* a symbol, which is precisely the shared-region shape that made `body` grow quadratically: a file-level doc comment attributed to every symbol in the file reproduces that bug exactly. Bounding it at the same choke point closes the hole before a future extractor opens it. */
export function boundSymbolDocstring(docstring: string): string {
  ...
  // Budget the marker inside the cap, not on top of it: appending it to a full-length slice would make the stored value exceed the very bound this constant declares, which quietly turns a hard storage limit into an approximate one.
  let end = MAX_SYMBOL_DOCSTRING_CHARS - DOCSTRING_TRUNCATION_MARKER.length
  // Never cut between the halves of a surrogate pair -- that stores a lone surrogate, which is not valid text and can surface as a replacement character or upset consumers downstream.
...
// precedingDocComment / DocCommentStyle live in doc_comment.ts (shared with languages/common.ts, which parser.ts itself imports from -- defining them here would create an import cycle). See that module's doc comment. Re-exported here so existing importers of `parser.js` keep working.
...
/** Is tree-sitter (binding + grammar) available for `lang`? Returns `false` rather than throwing when the native binding or a grammar package is missing, so callers can branch to the regex fallback. Languages without a bundled grammar (markdown, json, yaml, toml, css, dockerfile, bash, unknown) are always `false`. */
...
/** Parse one file and return its extracted symbols + refs. Dispatches to the tree-sitter extractor when a grammar is available for the detected language, otherwise falls back to regex. Unknown languages and unreadable files yield empty symbol/ref lists (never throws). Call-site refs are extracted for the tree-sitter languages in {@link REF_LANGUAGES}; the regex-fallback and structured-config languages yield no refs. */
...
/** Symbol extraction for languages with no tree-sitter grammar: the regex and structured-config adapters. Returns an empty list for `unknown`. */
...
/** Delete every index row (symbols, refs, files) for one file. On a case-insensitive filesystem the path match folds case — mirroring index_reader's pathEq — so rows written under a different path casing by a prior reindex are removed rather than orphaned as case-variant duplicates. */
...
/** Write a parsed result's rows into the index DB, replacing any prior rows for the file in a single transaction (DELETE + INSERT, matching the Python bulk-replace strategy) so a re-index never leaves stale symbols behind. Called by {@link indexFileSync}, the worker drain loop's synchronous entry point. */
function writeParseResult(
  ...
  // embed_sha is the OTHER freshness key this same row carries (see files.embed_sha / makeIndexer in worker.ts): it answers "were the current chunks/vectors embedded from this exact content", independent of files.parser_sha answering "did this extractor version write the symbol/ref rows below". A parser-only reparse -- this write happening because parser_sha was stale while the bytes on disk never moved -- must not silently reset that independent answer to unknown. Read the prior row BEFORE deleteFileRows below removes it: once the DELETE runs there is nothing left to read this from, and the whole point is to carry it across that delete.
  const priorRow = db
  ...
  const writeAll = db.transaction(() => {
    ...
    for (const s of result.symbols) {
      ...
      // Bound every stored body regardless of which extractor produced it -- see MAX_SYMBOL_BODY_CHARS / boundSymbolBody. This is the single choke point through which all parsed symbols reach the DB, so capping here makes an unbounded-body bug in any one language extractor incapable of bloating global.db. An over-cap body is stored empty, not truncated, so resolveBody still serves the complete symbol from source on read.
      insSym.run(
        s.filePath,
        s.name,
        s.kind,
  ...
  // `.immediate()` -- BEGIN IMMEDIATE -- rather than a plain call, which the driver issues as a deferred BEGIN. A deferred transaction takes its read snapshot first and only asks for the write lock when it reaches a writing statement, and SQLite refuses that upgrade with SQLITE_BUSY straight away instead of consulting the busy handler, so `busy_timeout` does nothing for it. Six `index` runs starting together against a database that did not exist yet reproduced it every time: one of them failed on its very first file with "database is locked", dropped that file from the index, and still exited 0. Raising busy_timeout to 60s in a run lasting one second changed nothing, which is what proved the handler was never being asked. BEGIN IMMEDIATE takes the write lock up front, where the busy handler does apply.
  writeAll.immediate()
}

/** Synchronous index: read, parse, and write one file's rows in a single call. The worker drain loop runs synchronously — it clears the dirty queue only after the batch has been written — so it needs a synchronous entry point. Reads with `readFileSync`, runs the same `parseContent` extractor, and shares {@link writeParseResult}. A file genuinely gone (ENOENT — deleted in the race window between being fingerprinted and this read) is skipped silently, never throws. Any other read failure (EBUSY/EPERM/EACCES from an AV or editor file lock, EIO, ...) is rethrown, so it reaches {@link makeIndexer}'s catch in worker.ts, which logs it and returns the `INDEX_FAILED` sentinel instead of letting `processDirtyBatch` silently count this file as indexed. */
// `preReadBytes`, when passed, is used verbatim instead of this function opening `filePath` itself. This exists so a confinement-pinned caller (read_commands.ts's healStaleIndex / force-refresh paths) can verify the file's identity against its MCP-validated pin BEFORE any bytes are read, then hand those already-verified bytes straight through -- closing the check-then-open race that a second, independent fs.readFileSync inside this function would reopen. A CLI caller (worker.ts, cli.ts) never has a pin to verify against and omits this parameter, so this function's own fs.readFileSync (below) still runs for every call site except the pinned ones, unchanged from before this parameter existed.
export function indexFileSync(rawPath: string, dbPath: string = globalDbPath(), preReadBytes?: Buffer): void {
  // Every symbols/refs/files row this call writes derives from this one spelling, so the whole parse side of the index agrees on a single name for the file. See canonicalizeIndexPath.
...
/** Prefix used to stamp `files.embed_sha` when {@link indexFileEmbeddings} early-returns because `indexing.embeddings_enabled` is off, instead of the file's real content sha. Kept distinct from a real embed_sha (see {@link indexFileEmbeddings}'s own doc comment) so re-enabling embeddings later can't be mistaken for "already embedded, unchanged" by makeIndexer's embedUnchanged gate in worker.ts, which checks the CURRENT embeddings_enabled state and only treats this disabled-marker form as "unchanged" while still disabled -- a bare sha match here would otherwise permanently skip re-embedding once a user turns embeddings back on for content that was only ever marker-stamped, never actually embedded. */
...
let disabledStamps = 0

/** How many `disabled:` markers {@link indexFileEmbeddings} has stamped in this process. The worker compares it across idle cycles: a marker stamped since the last one means embeddings were off at some moment in between, even when every cycle that looked found them on. */
export function disabledStampCount(): number {
  return disabledStamps
}

/** Prefix used to stamp `files.embed_sha` when {@link indexFileEmbeddings} could not actually embed a file because the optional embedding deps were absent (the inference runtime or the sqlite-vec `chunk_vectors` table). Distinct from a real embed_sha so the freshness gate can re-embed the file once the deps are installed -- otherwise a project indexed on a deps-less install would stamp bare shas, and every previously-indexed unchanged file would look "already embedded" the instant the model is added, leaving the semantic index permanently empty for that content. Distinct from {@link DISABLED_EMBED_SHA_PREFIX} because the two cases clear on different conditions: disabled clears when config re-enables embeddings, unavailable clears when the deps become installable/usable (see {@link isEmbedFresh}). */
...
/** Prefix used to stamp `files.embed_sha` when {@link indexFileEmbeddings} skipped a file only because it is larger than `indexing.large_file_symbol_only_kb`. Third instance of the same shape as {@link DISABLED_EMBED_SHA_PREFIX} and {@link UNAVAILABLE_EMBED_SHA_PREFIX}, and it clears on a third condition: the threshold itself, which is a user-tunable config value rather than a property of the content. A bare sha here reads as "really embedded, nothing left to do", so raising the threshold left every previously-skipped file looking permanently fresh and the background worker never re-embedded any of them -- `semantic` stayed blind to that content until something edited it, while `symbol` and `read` worked normally. `token-goat doctor`'s embedding coverage remediation tells users to raise exactly this value, so the case is reached by following the product's own advice. The threshold in force at stamp time is encoded in the marker so a later change to it no longer matches and the file is re-examined; see {@link isEmbedFresh}. */
...
/** Prefix used to stamp `files.embed_sha` when {@link indexFileEmbeddings} gave up on a document because its extraction ran past {@link MAX_DOCUMENT_WORK_MILLIS}. Fourth instance of the shape {@link OVERSIZE_EMBED_SHA_PREFIX} introduced, and it exists for the same reason: the skip was conditional on a value that can change, so a bare sha would record it as terminal and the document would never be embedded again. A clock refusal is the weakest of all these conditions, because the value it is conditional on is not even a setting -- it is how busy this machine was for one minute. Stamping a bare sha for it, which is what this branch used to do, is exactly the "permanent verdict recorded from a temporary condition" the extraction-*failure* branch a few lines below refuses to do, applied to a case that only looked settled because the error happened to extend {@link DocumentRefusedError}. The bound in force is encoded so that raising the clock in a later release re-examines every document a lower one refused; see {@link isEmbedFresh}. */
...
/** Prefix used to stamp `files.embed_sha` when {@link indexFileEmbeddings} skipped a file because its extension names a format with no text in it (see NON_TEXT_ASSET_EXTENSIONS in asset_extensions.ts). Bears {@link NON_TEXT_ASSET_SET_ID} for the reason {@link TIMEOUT_EMBED_SHA_PREFIX} bears its work clock: the bound is a compiled-in list rather than a config value, and a release that edits the list must re-examine every file stamped under the old one -- otherwise removing an extension from the set would leave every file that already carried that extension buried forever, the same permanent-verdict-from-a-changed-condition failure a bare sha causes for the oversize case. */
...
/** Prefix used to stamp `files.embed_sha` when {@link indexFileEmbeddings} skipped a file that would have contributed more than `indexing.max_chunks_per_file` chunks. Threshold-bearing for the same reason {@link OVERSIZE_EMBED_SHA_PREFIX} is: the bound is user-tunable, so raising it has to re-examine every file the old value refused rather than leave them stamped as done. Distinct from the oversize marker rather than reusing it because the two measure different axes and a file can be well under the byte threshold while far over this one. Chunk cuts snap to structure, so a generated JSON snapshot of a few hundred kilobytes -- thousands of one-line keys, each its own boundary -- becomes thousands of near-identical chunks, while a source file of the same size becomes a few hundred meaningful ones. On one real index the twenty-odd worst files were all under the 500 KB byte threshold and held 22% of every chunk on the machine. */
...
/** Apply `maxChunks` to the rows a just-finished embed actually wrote, dropping them and stamping the over-cap marker when it went over. Returns whether it did, so the caller can skip its own stamp. The cheap count taken before embedding cannot decide this on its own. It cuts with `chunkFile`'s char budget, and `indexFile` re-cuts with the model's tokenizer before inserting whenever the embedding deps are present -- the branch that exists precisely when this cap matters. The token cut is strictly the finer of the two: it flushes on a token budget as well as a char one, it sub-splits any single line past that budget, and it stops merging below-floor chunks ("a short vector costs one more row", as the flush in embeddings.ts puts it). So the pre-count is a lower bound, and a file can clear a 600-chunk ceiling on the coarse cut and store several hundred more than that on the fine one. The pre-count is kept because it is free and its rejections are all correct; it is just not the last word. Counting stored rows is also what makes this agree with `selectUnembeddableChunkFiles` (embed_backfill.ts), which sweeps on exactly this measure. When the two used different measures they could reach opposite verdicts on one file, and which one won came down to whether that sweep's version ledger had been stamped yet. */
...
/** Return `absPath` with its final segment spelled the way the filesystem actually spells it. The companion of `indexedPathSpellingIsStale`. That guard notices when a stored row's spelling has drifted from disk and forces a reparse -- but a reparse only helps if the reparse writes a *different* spelling than the one already stored. Every writer used to hand its own caller's path straight to `writeParseResult`, and no caller's spelling is authoritative: `git ls-files` reports the name in git's index, which still says `MixedName.ts` after an unstaged case-only rename on a case-insensitive filesystem, because git never sees such a rename at all. So the bulk `index` command detected the drift, reparsed, wrote the same stale name back, and detected the very same drift again on the next run -- reparsing and re-embedding an unchanged file on every single run, forever, while every citation named a path with no directory entry behind it. Canonicalizing here, at the one point every writer passes through, is what makes that converge. Only the last segment is taken from the filesystem. `fs.realpathSync.native` canonicalizes every parent directory too, so adopting its whole answer would rewrite rows to an ambient prefix spelling the caller never used (`C:/WINDOWS/TEMP/...` for a caller that said `C:/Windows/Temp`), churning paths for a difference that is not the file's own. The directory prefix stays exactly as the caller spelled it. Three rails keep this from ever *relocating* a path. `fs.realpathSync.native` also resolves symlinks and Windows junctions, so it can answer with a completely different file: the new segment is adopted only when it case-folds equal to the one it replaces, and only when the resolved file's directory case-folds equal to the directory the caller named. And any failure returns the caller's path untouched. `fs.realpathSync` (without `.native`) is useless for this: on Windows it echoes the caller's own spelling back. */
export function canonicalizeIndexPath(rawPath: string): string {
  ...
  try {
    ...
    // Unreadable or gone right now -- a git-tracked file deleted from the worktree reaches the indexer on every run. Keep the name callers know it by rather than guess at a spelling.
    return absPath
  }
  // Split on the caller's own string, not a normalized copy: normalizePath lowercases a UNC host and share, so rebuilding the prefix from it would rewrite `//SERVER/Share/...` to `//server/share/...` on the runs that correct a name and leave it alone on the runs that do not -- a second spelling difference introduced by the very function meant to remove one.
  ...
  // The resolved file must live in the directory the caller named. `fs.realpathSync.native` also follows symlinks and Windows junctions, so `alias.ts` can resolve to `../elsewhere/Alias.ts` -- fold-equal basenames, entirely different file. Adopting that spelling would name a directory entry that does not exist where the row says it does.
...
/** True when the index's stored spelling of `absPath` no longer matches the file's real spelling on disk. Only ever true on a case-insensitive filesystem, which is where the problem lives. {@link getFileEntry} finds a row by folded path, so after `mv b.ts B.ts` the lookup still hits and the content is byte-identical -- the sha gate skips the file, and the row goes on saying `b.ts` forever. Nothing else corrects it: the file still exists, so the deletion sweep leaves it alone, and only a later change to the file's *content* rewrites the row. Until then every `symbol`, `read`, `refs` and `map` answer names the path with a spelling it no longer has, which is the one thing this tool exists to get right. The real spelling is read from the filesystem rather than taken from the caller, because no caller's spelling is authoritative: the walk reports the dirent's, git reports the one in its own index (still the old one until the rename is staged), and the edit hook reports whatever the editor passed. Trusting the caller would let two of them rewrite the row back and forth on alternate runs. Deliberately narrowed to a pure case difference, by the fold comparison against the resolved spelling on the last line. `realpathSync` also resolves symlinks and Windows junctions, so a project reached through a link resolves to a structurally different path -- without that comparison every file in such a project would read as stale on every run and the whole tree would reindex forever. The identical fold check earlier is a cheap filter for a caller naming a structurally different path, not a second correctness guard. This costs one realpath per file per index run, which is noise beside the full-file read and hash `fingerprintFile` already performs for the same file on the same pass. A file whose spelling has actually drifted pays for two more, one in each writer (see canonicalizeIndexPath) -- three in total, on the one run that corrects it and never again. */
export function indexedPathSpellingIsStale(storedPath: string, absPath: string): boolean {
  if (!isCaseInsensitiveFs()) return false
  // Both sides go through normalizePath, and the stored side is not exempt: the incremental worker writes the dirty queue's own spelling straight through, so a row indexed by the daemon can carry an upper-case drive letter (`C:/...`) where the bulk `index` command's rows carry the lower-case one normalizePath produces. Comparing a raw stored value against a normalized candidate makes that difference look like a stale spelling, and the file then reindexes on every single drain, forever.
  const stored = normalizePath(storedPath)
  const candidate = normalizePath(path.resolve(absPath))
  // Cheap structural filter only. Deliberately NOT `storedPath === candidate`: the caller's own spelling agreeing with the stored one proves nothing, because the caller may simply be repeating the same stale spelling the row already holds -- git reports the name in its own index, which still says `readme.md` until a case rename is staged, and an editor hook reports whatever the editor passed. Short-circuiting on that agreement is how the row would stay stale forever in exactly the cases that never reach a filesystem walk.
  ...
  // Both real and stored have the same folded path, but real was canonicalized from the filesystem root by fs.realpathSync.native (expanding canonical case for all parent directories, e.g. C:/Windows/Temp vs C:/WINDOWS/TEMP), whereas stored/candidate may share an ambient directory prefix. Compare segments: if the filename (last segment) differs, the file was renamed.
...
/** The shared read side of the embed-freshness gate used by both worker.ts::makeIndexer and cli.ts's bulk index loop. Returns true when the file's stored `embed_sha` already represents the correct terminal embedding state for the CURRENT environment, so re-running indexFileEmbeddings would do no useful work and can be skipped: - embeddings config-disabled: fresh only when stored is the `disabled:` marker for this sha. - enabled + a bare sha match: fresh (the file was really embedded, or was empty / policy-skipped with nothing to embed -- both are terminal regardless of deps). - enabled but deps currently unavailable: an `unavailable:` marker for this sha is also fresh, so an unchanged file is not re-entered on every worker drain while deps stay missing. - enabled + deps available: an `unavailable:` (or `disabled:`) marker is NOT fresh, forcing the real first embed now that it can finally succeed. - enabled + an `oversize:` marker: fresh only while `symbolOnlyKb` still matches the threshold the marker was stamped under, so raising `indexing.large_file_symbol_only_kb` re-examines the file instead of leaving it permanently skipped. See {@link OVERSIZE_EMBED_SHA_PREFIX}. `symbolOnlyKb` is the caller's current `indexing.large_file_symbol_only_kb`. Pass 0 when no config is in hand: config validation floors that key at 1, so 0 matches no stamped marker and the file is re-examined rather than assumed current, which is the safe direction. */
...
/** `sha`, when provided, is stamped into `files.embed_sha` after {@link embedIndexFile} commits successfully -- tracked separately from `files.sha` (the parse-freshness gate) so a crash or thrown error mid-embedding never gets masked by the parse-sha gate: the embed_sha column is left at its previous (stale/empty) value on any early return or thrown error below, so a later touch of byte-identical content still re-triggers embedding instead of being permanently sha-gate-skipped. See makeIndexer in worker.ts for the read side of this gate. */
export async function indexFileEmbeddings(
  ...
  // Same canonicalization as indexFileSync, for the same reason: chunk rows must be keyed by the spelling the parse side wrote, not by whatever the caller happened to pass.
  const filePath = canonicalizeIndexPath(rawPath)
  const ixCfg = loadConfig().indexing
  ...
  if (isEmbeddableDocument(filePath)) {
    // Binary document formats (PDF/DOCX/PPTX/XLSX) need format-specific extraction, not a raw utf8 read of the file's bytes -- must run before the generic read below, which would otherwise reinterpret binary content as garbage text. detectLanguage() returns 'unknown' for these extensions (no Language union member, no code symbols), so none of the ipynb/large-file/salesforce branches above or below apply to them.
    ...
    try {
      ...
      if (!isDocumentRefusal(err)) {
        // Left unstamped on purpose, the same way a thrown embedIndexFile is below: this file's extraction failed rather than declined, and a stamp here would be a permanent verdict recorded from a temporary condition -- pdfjs briefly missing, the file mid-write -- so the document would never be embedded again even once the cause was gone.
        onError?.(err)
        return
      }
  ...
  // A dotenv file's values are secret by the file's nature, and this is the one indexing path that persists raw file text: the symbol table already stores env keys with empty bodies, but chunks held the whole file, so `semantic` served the password. Redact before chunking, not at search time, so nothing sensitive is written to disk in the first place. See dotenv_redact.ts.
...
/** Stamp `files.embed_sha` for `filePath`, but only when a `sha` was actually provided (the incremental worker/CLI paths always pass one; some callers do not). `makeValue` derives the value to store from the (now-defined) sha -- identity for a real/terminal embed, or a disabled:/unavailable: marker. Centralizes the UPDATE so every early-return in {@link indexFileEmbeddings} records its terminal state identically. */
