/**
 * session_start hook: re-inject a short command-routing reminder every time a session starts, resumes, or restarts after compaction.
 *
 ...
 *
 * Deliberately omits the exact symbol count: this string lands in the earliest, most cacheable position of a SessionStart request (the part a provider's prompt/prefix cache matches on), and `countSymbols()` drifts on every reindex -- a live number here would invalidate that cache prefix every session, and every time the index changes mid-session. "Is indexed" is the only signal an agent acts on; the count was decoration in the worst possible position. Byte-identical across reindexes by construction: nothing in this string depends on index state beyond the ok/not-ok branch already selected by the caller.
 */
 ...
 *
 * Both the reminder text and the drift sweep branch on this, and it is computed once and passed to both rather than derived twice: two `countSymbols` calls would double a DB round trip on the session-start path, and a second call could disagree with the first if the worker committed a reindex between them.
 */
 ...
 *
 * Returns a one-line note when drift was found, or null when the index is already correct -- which is the overwhelmingly common case, and stays silent so the session-start context does not grow a line that says nothing. Only runs against an already-indexed project: on an unindexed one every tracked file is legitimately absent from the index, so the sweep would report the entire repository as drift and enqueue it, which is `token-goat index .`'s job and not a hook's.
 *
 * Never throws. Its caller is a session-start hook, and a sweep that failed is a missed repair, not a reason to degrade the reminder the hook exists to deliver.
 */
