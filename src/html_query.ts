...
export function formatHtmlOutline(summary: HtmlOutlineSummary): string {
  ...
  if (summary.doctype) lines.push(`DOCTYPE: ${summary.doctype}`)
  ...
  if (summary.headings.length > 0) {
    ...
    for (const h of summary.headings.slice(0, 30)) {
      lines.push(`  ${'  '.repeat(h.level - 1)}[h${h.level}] ${h.text} (line ${h.line})`)
    }
  ...
  if (summary.landmarks.length > 0) {
    ...
    for (const l of summary.landmarks.slice(0, 25)) {
      const idStr = l.id ? `#${l.id}` : ''
      const classStr = l.class ? `.${l.class.trim().split(/\s+/).join('.')}` : ''
      lines.push(`  <${l.tag}${idStr}${classStr}> (line ${l.line})`)
    }
  }
  ...
  if (summary.forms.length > 0) {
    ...
    for (const f of summary.forms) {
      ...
      lines.push(`  <form${idStr} method="${f.method || 'GET'}" action="${f.action || ''}"> (line ${f.line})`)
    }
  }
