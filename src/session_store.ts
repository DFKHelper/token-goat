...
export function sessionSidecarPath(sessionId: string, suffix: string): string | null {
  ...
  // Only the session state itself may end in `.json`: every reader that lists this directory (findLatestSessionId, listSiblingSessionStates, the manifest cleanup) takes a `.json` file to be a session, so a `.call-streak.json` sidecar resolved "the latest session" to an id nobody ever used.
  if (suffix !== '.json' && suffix.endsWith('.json')) return null
...
/** Parse one entry from a Python-format `files` dict value into a {@link FileEntry}. Returns null for any malformed entry so the caller can skip it safely. Python fields: rel_or_abs (string), read_count (int), last_read_ts (float seconds), read_size (bytes), last_edit_ts (float seconds, may be absent). The dict key itself is used as the path fallback when rel_or_abs is missing. */
...
/** Read every sibling subagent session-state blob for `sessionId` (i.e. every on-disk file salted with `${sessionId}:agent:${agentId}` per relay.ts's `sessionStateKey`), without touching in-memory session state. `sessionPath` sanitizes and truncates the *whole* salted key to 64 chars, so a sibling's filename is `${sanitize(sessionId)}_agent_<agentId prefix>` possibly truncated mid-agentId — but the `${sanitize(sessionId)}_agent_` prefix itself (well under 64 chars for realistic session ids) always survives intact, which is what this scan matches on. Returns [] when the id is empty/unusable, the sessions dir doesn't exist, or on any read error (fail-soft, mirroring every other read in this module). */
...
/** Load the persisted state for `sessionId` into the in-memory session maps. A clean session when no file exists or the id is empty/unusable, never the previous one's: the pi, opencode and OpenClaw bridges call relayInProcess inside their long-lived host process, which serves one session after another (pi re-fires session_start with a new id on new, resume and fork). Fail-soft: a corrupt file leaves the session empty rather than throwing. */
