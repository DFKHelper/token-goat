...
//
// `shell: true` is deliberate — the original command may use pipes, redirects, globs, env expansion, or chaining the user wrote intentionally. The command string was generated by the agent/user, not an untrusted source, and the hook's `detectFromCommand` already rejected pipelines/subshells before wrapping, so the wrapper adds no further escaping.
