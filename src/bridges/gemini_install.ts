/** Gemini CLI install / uninstall writer. `token-goat install --gemini` writes hook entries into `~/.gemini/settings.json` in addition to the base Claude Code install (see README's "Gemini CLI users" section). This module only ever touches that one file -- the base Claude Code writer in `../install.ts` is unaffected and is always run separately by the caller, exactly like `../bridges/codex_install.ts`'s `installCodex` and `./pi_install.ts`'s `installPi`. Gemini CLI's own hook config (verified against https://geminicli.com/docs/hooks/reference/) uses the same `hooks: Record<Event, Array<{ matcher?, hooks: [...] }>>` shape Claude Code's own `settings.json` uses (see `HookMatcherGroup`/`HookCommandEntry` in `../install.ts`), just under different event keys and with tool-name matchers that are genuine regexes (Gemini docs: "matchers are Regular Expressions" for `BeforeTool`/`AfterTool`; the lifecycle event `PreCompress` takes no matcher and fires on every occurrence). No shim script is needed the way Codex's strict `additionalProperties: false` schemas require one: Gemini's hook command invokes `token-goat hook <event>` directly, without any response reshaping. This was re-verified directly against the raw `docs/hooks/reference.md` in google-gemini/gemini-cli on GitHub (2026-07-09, gemini CLI itself not installed on this machine): a `BeforeTool` deny accepts `{"decision":"deny"|"block","reason":"..."}` at the top level with no `hookSpecificOutput` wrapper -- exactly what `serializeOutput` (`../hook_registry.ts`) already emits for every deny on every harness, "block" being a documented alias for "deny". See `tests/relay.test.ts`'s "relay Gemini deny wire format" suite, which exercises this against a real production deny handler (hooks_mcp.ts's MCP-call dedup) through the real `relay()` path rather than assuming it. It is invoked via the absolute Node binary (`process.execPath`) and the running token-goat entry path (`process.argv[1]`), not a bare `token-goat` command -- {@link geminiHookCommand} -- for the same reason `hookCommandFor` in `./codex_install.ts` and `./copilot_cli_install.ts` do: a global npm install on Windows resolves `token-goat` to a `.cmd`/`.ps1` shim, which a `command`-type hook spawned without `shell: true` cannot exec, and Gemini's own hook runner is not documented to set that. `relay.ts`'s `harnessForNormalization()` routes the resulting `token-goat hook` subprocess to `normalizePayload(..., 'gemini')` whenever it inherits Gemini CLI's own environment (`GEMINI_API_KEY` / `GOOGLE_API_KEY`, via `detectHarness()`), which is what actually translates Gemini's snake_case tool names to token-goat's canonical ones at runtime -- this module only has to get the *installed command* right. Event -> internal-tool coverage is derived from {@link GEMINI_TOOL_NAME_MAP} (`../hooks_cli.ts`, exported so both the runtime payload normalizer and this installer share one source of truth) grouped by which internal tool actually has a registered `pre_tool_use`/`post_tool_use` handler -- queried live from {@link toolMatcherFor} (`../hook_registry.ts`) rather than a hand-maintained list. A hand-maintained list is exactly what went stale twice here already: Gemini's `glob` tool name mapped to `Glob`, which gained pre/post handlers in hooks_glob.ts on 2026-07-18, seven weeks after the original hardcoded set was written and silently missed the addition; a hardcoded set also once excluded `WebSearch` entirely, forcing google_web_search through the unrelated WebFetch handler as a workaround (see GEMINI_TOOL_NAME_MAP's `google_web_search` entry in `../hooks_cli.ts`) since that was the only web-ish name the old set allowed. Deriving from the live registry the same way `../install.ts` already narrows Claude Code's own matcher makes the next missed handler mechanically impossible instead of merely tested for -- see `registeredInternalTools` below. This depends on every hook handler module already being imported (side effect of `../relay.ts`) before this module's install/uninstall functions run; `cmdInstall` in `../cli.ts` is responsible for that, exactly as it is for `../install.ts`'s own narrowing. A corrupt-but-recoverable `settings.json` (exists but fails to parse) must never be silently clobbered -- {@link installGemini} throws {@link GeminiSettingsParseError} before any write in that case, mirroring the `SettingsParseError` / `CodexConfigParseError` strict-mode guards in `../install.ts` / `./codex_install.ts`. */
...
/** Shape of the current exec-path-hardened invocation {@link geminiHookCommand} writes: a quoted absolute Node binary, a quoted absolute token-goat entry path (captured), and a trailing `hook <event>` call for one of the events this bridge wires. Matching this shape alone is NOT sufficient to identify the command as token-goat's own -- see {@link GEMINI_ENTRY_PATH_MARKER_PATTERN}. */
const GEMINI_COMMAND_PATTERN = /^"[^"]+"\s+"([^"]+)"\s+hook\s+(?:pre_tool_use|post_tool_use|pre_compact)$/

/** Literal marker identifying token-goat's own entry path: this package's name (`package.json`'s `name`/`bin` key, e.g. `dist/token-goat.mjs`). Any install layout -- a local dev checkout, a global `npm install -g`, or an install as another project's dependency -- places the entry script under an npm `node_modules/token-goat/...` directory (or, in this repo's own checkout, a path that itself contains a `token-goat` segment), so requiring this substring in the captured entry-path segment (anchored the same way {@link GEMINI_LEGACY_MARKER_PATTERN} is, so it can't match as part of a longer unrelated word) distinguishes our own hook command from an unrelated tool's same-shape command -- e.g. `"C:/some/other/node.exe" "C:/some/other/tool.js" hook pre_tool_use` has the identical shape but no `token-goat` path segment and correctly does not match. */
...
/** True when `command` is exactly `desiredCommand`, the *current* exec-path-hardened form {@link geminiHookCommand} produces for this event -- never the legacy bare `token-goat hook <event>` form, and never a same-shape command whose baked entry path is stale (an `npm install -g` reinstall, or switching between a local dev checkout and a global install, changes `process.argv[1]` -- a prior install's baked path still matches the generic {@link GEMINI_ENTRY_PATH_MARKER_PATTERN} marker, so only an exact string comparison against the freshly-computed command can tell current from stale). Distinguishing this from {@link isGeminiTokenGoatCommand} (which matches ANY token-goat entry, legacy or stale-path or current) lets {@link installGemini} tell "already upgraded and current" apart from "still on the old, `.cmd`-shim-unsafe command, or pointing at a path that no longer exists" so a re-install actually repairs a stale/legacy entry instead of treating it as sufficient to skip. */
function isCurrentGeminiTokenGoatCommand(command: string, desiredCommand: string): boolean {
  if (typeof command !== 'string') return false
  return command === desiredCommand
}

/** Build the shell command Gemini should run for one hook entry. Invoked via the absolute Node binary (`process.execPath`) and the running token-goat entry path (`process.argv[1]`), not a bare `token-goat` command -- see the module doc comment and `hookCommandFor` in `./codex_install.ts` / `./copilot_cli_install.ts` for the Windows global-install rationale. Falls back to the old PATH-based bare command when `process.argv[1]` is unavailable (should never happen under a real `node <script>` invocation). */
...
/** Group {@link GEMINI_TOOL_NAME_MAP}'s Gemini tool names by their mapped internal tool, preserving the map's own key order (so output is deterministic across runs). */
...
/** Desired matcher list for one Gemini hook event. `PreCompress` is a lifecycle event: a single no-matcher group (`undefined`) that fires on every occurrence, per Gemini's own docs. `BeforeTool`/`AfterTool` get one regex-alternation matcher per internal tool that actually has a registered handler for that (event, tool) pair (see {@link registeredInternalTools}), e.g. `^(read_file|read_many_files|list_directory)$` for Read. */
...
/** Install the Gemini CLI integration. Always additive: never touches Claude Code's `~/.claude/settings.json` (the caller is responsible for also running the base install). Idempotent -- a second call reports `alreadyInstalled: true` and does not duplicate any matcher group. */
export function installGemini(): GeminiInstallResult {
  const p = geminiSettingsPath()
  // strict: true -- a settings.json that exists but fails to parse must abort before any write (see GeminiSettingsParseError), not silently proceed as if it were empty and get clobbered below.
