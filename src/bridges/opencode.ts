/** opencode bridge plugin. Unlike Codex (had an existing `CODEX_HOOK_SCRIPT` template) and pi (had a whole real extension already sitting unreferenced in the repo), no opencode bridge code existed anywhere in this repo before this file. It was authored fresh, verified directly against opencode's real source (github.com/anomalyco/opencode -- the project formerly at sst/opencode; GitHub transparently redirects the old path) and https://opencode.ai/docs/plugins/, not secondhand descriptions. opencode's plugin API (packages/plugin/src/index.ts) exposes three hooks relevant here: - tool.execute.before(input: {tool, sessionID, callID}, output: {args}) => Promise<void> - tool.execute.after(input: {tool, sessionID, callID, args}, output: {title, output, metadata}) => Promise<void> - experimental.session.compacting(input: {sessionID}, output: {context, prompt?}) => Promise<void> There is no pre-read context-injection hook like Claude Code's PreToolUse additionalContext -- tool.execute.before can only mutate output.args (or throw to abort the call; opencode's own docs demonstrate this with `throw new Error("Do not read .env files")` inside tool.execute.before, and community plugin guides show the same pattern for blocking `rm -rf`). This is exactly why README's "opencode users" section says session hints don't work here: there is no channel to inject text into the model's context before a read happens. Image shrinking still works despite that gap. token-goat's pre_tool_use response for a shrunk image is a `context` output -- `{hookSpecificOutput:{additionalContext: "<summary>\ndata:image/<fmt>;base64,<data>"}}` (see preReadImageHandler in image_shrink.ts) -- built for Claude Code's additionalContext channel, which opencode has no equivalent of. Rather than drop the feature, this plugin decodes the embedded data URL, writes it to a temp file, and rewrites output.args.filePath to point there, so opencode's own `read` tool ends up reading the shrunk copy instead of the original. Real opencode built-in tool ids and parameter keys, confirmed directly against source (not the tool's file name or display name): read:     filePath, offset, limit                  (tool/read.ts) write:    filePath, content                          (tool/write.ts) edit:     filePath, oldString, newString, replaceAll (tool/edit.ts) bash:     command, timeout, workdir                  (tool/shell.ts registers under ShellID.ToolID, which -- despite the file being named shell.ts -- is literally the string "bash": tool/shell/id.ts) grep:     pattern, path, include                     (tool/grep.ts) glob:     pattern, path                              (tool/glob.ts) webfetch: url, format, timeout                       (tool/webfetch.ts) websearch: query, numResults, ...                    (tool/websearch.ts, v1.18.16) skill:    name                                       (tool/skill.ts, v1.18.16) task:     prompt, subagent_type, description, ...    (tool/task.ts, v1.18.16) `token-goat hook <event>` only accepts the exact snake_case event names in HOOK_EVENTS (src/types.ts): pre_tool_use, post_tool_use, pre_compact, notification, stop, user_prompt_submit, subagent_stop -- relay() (src/relay.ts) checks the event name against that closed set via isHookEventName() and no-ops to `{}` on anything else. This plugin uses those exact names (unlike PI_EXTENSION_SCRIPT's pre-existing "session-start" / "pre-read" / "post-bash" style event names, none of which match HOOK_EVENTS -- every one of those calls currently no-ops). Response contract from `token-goat hook <event>` (src/hook_registry.ts serializeOutput), all confirmed by reading the handlers that actually produce them: - deny:   {"decision":"block","reason":"..."} - context (pre_compact / notification): {"systemMessage":"..."} - context (other events): {"hookSpecificOutput":{"hookEventName":"...","additionalContext":"..."}} - rewriteInput (pre_tool_use only, e.g. Bash command compression): {"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"allow","updatedInput":{...}}} - rewriteOutput (post_tool_use only, e.g. WebFetch fencing/redaction, output compression): {"hookSpecificOutput":{"hookEventName":"PostToolUse","updatedToolOutput":"..."}} - pass: {} This list is the complete producer set from serializeOutput, on purpose: an earlier revision enumerated only four of the five shapes, and the missing one (rewriteOutput) silently shipped dead -- tool.execute.after applied the context append and nothing else, so injection fencing, secret redaction and output compression never reached an opencode session's model. */
