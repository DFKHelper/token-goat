/** OpenClaw bridge plugin. OpenClaw uses an in-process TypeScript plugin system (not a subprocess-hook config format like Codex/Gemini): a `.ts` file registered in `~/.openclaw/openclaw.json`, loaded and executed by the OpenClaw gateway itself. This template is dropped onto disk verbatim by `./openclaw_install.ts`'s {@link installOpenclaw}; it is never imported or executed by token-goat itself. Verified against two independent sources: - A live fetch of docs.openclaw.ai/plugins/hooks, /plugins/sdk-setup, and the gateway configuration reference (this session): confirms `before_tool_call`/`after_tool_call` exist with the exact block/rewrite-params shape used below, `definePluginEntry` is the real registration entry point, and `plugins.load.paths` + `plugins.entries.<id>` (no `path` field -- the plugin's own declared `id` is the join key) is the current two-part config schema `./openclaw_install.ts` writes. - An earlier, independent verification against OpenClaw's own `aristotle-agent/types.ts` source (prior session, `reference-openclaw-api` memory): confirms `register()` must be synchronous (`api.on()` handlers may be async) and the `before_tool_call` event shape (`{toolName, toolCallId, params}`). That pass also claimed `event.params` keys were already snake_case (`file_path`, `command`, ...), which turned out to be wrong for the path-carrying tools: OpenClaw's own tool input schemas (openclaw/openclaw src/agents/sessions/tools/read-tool-contract.ts, edit.ts, bash.ts) declare `path` for read/edit/write and `command`/`timeout` for bash. So `command` really did line up with token-goat's key, but `file_path` never arrived, and every Read/Edit/Write hook that calls getFilePath() (re-read denial, image shrink, post-edit indexing) silently no-opped. PATH_ARG_TOOLS/toToolInput below fix that by ADDING the canonical `file_path` alongside the original `path` (the Copilot shim's remapToolInput convention) rather than renaming it -- renaming an already-correct key is the bug the Gemini bridge shipped with initially (`GEMINI_INPUT_KEY_MAP`), and keeping the original key means a wrong mapping degrades to the old no-op rather than corrupting the call. What's still genuinely unverified against a live OpenClaw instance (no instance was available this pass -- see README's "openclaw users" section and this bridge's own follow-up note): the exact current built-in tool-name list (the sources disagree -- `grep`/`glob`/`webfetch`/`apply_patch`/`exec` per the older source vs. `web_search`/`web_fetch` per the fresher docs vs. `read`/`bash`/`edit`/`write`/`grep`/`find`/`ls` per openclaw/openclaw's own sessions/tools ToolName union, whose bash.ts registers `name: "bash"`, not `exec`; TOOL_TO_TG below includes every spelling, since an unmatched key is harmless no-op, not a wrong guess), and whether tool-call context (`ctx`) actually carries a stable per-session identifier (the older source says OpenClaw "has no session concept"; this session's fresh research found `ctx.sessionId`/`ctx.sessionKey` on general agent hook contexts -- both are tried below, falling back to a per-process pseudo-id if neither is present). Compaction hooks (`before_compaction`/`after_compaction`) are observation-only in OpenClaw's SDK: there is no return-value channel to inject a manifest into the next turn the way pi's `session_compact` -> `pi.sendMessage(...)` does. This bridge still calls `pre_compact` so token-goat's own session-cache bookkeeping stays in sync with real compaction events, but it cannot re-inject a manifest. */
