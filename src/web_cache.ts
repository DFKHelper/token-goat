...
/** Derive the deterministic cache id for `url`. The id is a 16-hex-char prefix of the URL's SHA-256, matching the short-hash convention used across the Python cache layer (`cache_common.py`, `bash_cache.py`). Deterministic so the same URL maps to the same id within and across sessions. */
function cacheIdForUrl(url: string): string {
  return shortFingerprint(url)
}

/** Store `content` for `url` and return its cache id. Re-storing the same URL overwrites the body and keeps the same id, so the URL index always points at the latest content. `dedupKey` (default: `url`) is what the cache id is derived from. A caller whose identity for "is this the same request" is broader than the bare URL (e.g. hooks_fetch.ts, where a WebFetch answer is specific to the `prompt` asked, not just the URL) can pass a composite key so two requests for the same URL with different keys get distinct ids - while the blob's stored `url` field (and `_urlIndex`, used by getWebOutputByUrl / wasUrlFetchedThisSession) stay keyed on the real URL for display and URL-only lookups. `rawContent`, if given and different from `content`, is the body as actually fetched, before hooks_fetch.ts's extractCleanText cleaning pass. Stored alongside the cleaned `content` so the cache stays lossless (recoverable via {@link getWebOutputRaw} / `web-output --raw`) even though the default read path still returns the cleaned text. */
export function storeWebOutput(url: string, content: string, dedupKey: string = url, rawContent?: string): string {
  ...
  // Redact once and reuse everywhere -- storeBlob() applies its own defense-in-depth redaction pass to the JSON it writes to disk, but the in-memory _byId cache and the recall index write below both bypassed that pass entirely (served/indexed raw content), leaking secrets via same-process reads and `token-goat recall`/FTS search. Redacting here keeps disk, in-memory, and the recall index all consistent.
  ...
  // The url itself can carry a secret (a signed URL's token query param, an embedded API key) just like content can -- redact ONLY the copy fed to the recall index (label/content), not the `url` used above for _urlIndex/storeBlob: those need the real url intact for exact-match lookups (getWebOutputByUrl) and display, and storeBlob() already applies its own whole-JSON redaction pass to what actually lands on disk there. Without this, the recall index (a separate, directly FTS-searchable table) indexed the raw url unconditionally, bypassing that protection -- the same gap already closed for content/output above.
...
/** Return the cached body for `cacheId`, or null if not present. Falls back to the disk store on an in-memory miss so a body cached by an earlier process (or run) resolves; a disk hit is cached in-process, and its URL (if recorded) re-populates the URL index. Returns null if the disk entry is older than DEFAULT_MAX_AGE_MS (stale cache). */
...
/** Return the cached body and id for `url`, or null if the URL was not fetched this session. In-memory only (by design — see `clearModuleCaches clears the in-memory maps` in web_cache.test.ts): does not read through to the disk store, so a URL fetched by an earlier, separate process will not resolve here even though its body is still on disk. For a cross-process/cross-invocation lookup keyed on a URL (each CLI run is a fresh process with no in-memory state), use {@link getWebOutputByUrlFromDisk} instead. */
...
/** Return the cached body and id for `url`, reading through to the disk store on an in-memory miss. `cacheIdForUrl` is a pure function of `dedupKey` (a content-address, not a value that needs to have been seen this process to compute), so the id can be derived directly and handed to {@link getWebOutput}, which already reads through to disk. This is what lets a URL-keyed cross-process caller (e.g. `gdrive.ts`, where each CLI invocation is a fresh process) recall a body persisted by an earlier invocation instead of always re-fetching over the network. `dedupKey` defaults to `url`, mirroring {@link storeWebOutput}'s own default -- pass the exact same `dedupKey` used at store time (e.g. a composite `${url}\x00${prompt}` key) so the id this function derives always matches the id `storeWebOutput` actually persisted under. */
