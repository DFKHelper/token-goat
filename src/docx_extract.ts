...
import { collectElements, collectParagraphTexts, collectTextRuns, decodeZipEntry, dropAlternateContentFallbacks, inlineMathRuns, NotAnOfficeDocumentError, ooxmlPartBudget, parseOoxmlPart, readOoxmlZip } from './ooxml_extract.js'
...
async function loadDocumentBody(filePath: string): Promise<unknown> {
  ...
  const parsed = await parseOoxmlPart(inlineMathRuns(inlineRunSeparators(xml), '<w:r><w:t xml:space="preserve">', '</w:t></w:r>'))
