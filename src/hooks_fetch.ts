...
import { passOutput, getToolName, getToolInput, denyOutput, extractToolResponseField, BODY_FIRST_TOOL_RESPONSE_KEYS, emitRewrite, emitRewriteIfChanged } from './hooks_common.js';
...
export function postFetchHandler(event: HookEvent): HookOutput {
  try {
    ...
    if (storedBody !== body) {
      ...
      // Measure the fenced string, which is what actually ships: gating on the unfenced length while emitting the fenced one is the same "gate(x), emit(x+extra)" over-report this branch's notice accounting was already fixed for once.
