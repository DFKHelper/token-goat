...
`)
  ...
  it('leaves bash-output alone, which caches local command output rather than fetched pages', async () => {
    const { storeBashOutput } = await import('../src/bash_output_cache.js')
    const id = await storeBashOutput('echo hi', `local output\n${PAYLOAD}\n`, 0, null)

    await runCli(['bash-output', id, '--full'])

    expect(stdout.join('')).not.toContain('untrusted-web-content')
  })
})
