...
/** A package that esbuild inlines into the bundle is never loaded from `node_modules` at run time, so listing it in `dependencies` makes every consumer download code the shipped artifact already carries. `html-to-text` and `exceljs` were moved out for exactly that reason; five more were missed at the time, and this guard is what stops any of the seven drifting back. The manifest half alone would be a test that mirrors the manifest, which proves nothing about behaviour. The half that matters reads the built bundle: if a future change marks one of these external, or reaches it through `createRequire` instead of a static import, the bundle starts resolving a package that is only a devDependency and every consumer install breaks at run time. That is the regression this file exists to catch, and it can only be seen in `dist/`. The converse is guarded too. `jsonc-parser` (reached via `createRequire`) really is resolved at run time, so demoting it ships a broken install. `better-sqlite3` is the third case and needs its own list. It used to be the largest thing a consumer installed; `src/sqlite_driver.ts` replaced it with Node's built-in `node:sqlite`, and it is now a devDependency kept only so `tests/sqlite_driver.test.ts` can diff the driver against it. So it must be in neither of the lists above: not inlined into the bundle, and not resolved from node_modules either. A static import of it anywhere in `src/` would break every consumer install while every test here still passed, because the repository's own tree has it. This asks "is this specific package resolved" of a list someone wrote. `bundle_runtime_imports_are_reviewed.test.ts` asks the same artifact the opposite question -- what is resolved, whoever thought to ask -- over the matcher both share in `bundle_specifiers.ts`. */
