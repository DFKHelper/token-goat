...
/**
 * Every package that may execute in a user's process, as resolved today.
 *
 * Adding a name here is the review. Before doing it, answer why this package is now in the runtime tree, what pulled it in, and whether the feature that needs it is worth the code it brings. Removing one needs no ceremony.
 */
const REVIEWED_PRODUCTION_PACKAGES: readonly string[] = [
  ...
  'node-fetch',
  'node-gyp-build',
  // A donation prompt, pulled in by tesseract.js and run from its postinstall. Carried because
  // tesseract.js requires it, not because anything here calls it.
...
describe('the set of packages that can run in a user process', () => {
  it('contains nothing that has not been reviewed', () => {
    ...
    // Non-vacuous: a lockfile that failed to parse, or a filter that excluded everything, would
    // otherwise report an empty tree and agree with any expectation at all.
  ...
  it('keeps SECURITY.md truthful about that surface', () => {
    // A published security claim that drifts is worse than no claim: a reader has no way to tell
    // it went stale, so it reads as current. This ties the sentence to the tree it describes.
  ...
  it('keeps the always-loaded surface at exactly one package', () => {
    // Everything else is `optional`, so it executes only when its feature is used on an install
    // that has it. This is the number that bounds the blast radius, and it should be argued over
    // before it moves.
    expect(
