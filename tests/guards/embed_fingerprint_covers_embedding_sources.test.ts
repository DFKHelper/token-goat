...
const NOT_EMBEDDING: Record<string, string> = {
  // The only entry here exempted by a mechanism rather than by irrelevance, so it is spelled out. An edit to this file CAN change what gets embedded -- that is what it is for -- but it carries its own invalidation instead of borrowing this digest's, on two fronts that together cover both directions a change can go. Removing an extension: every file skipped by it was stamped `asset:<NON_TEXT_ASSET_SET_ID>:<sha>`, isEmbedFresh compares that set id, so editing the set invalidates every asset stamp on the machine and each of those files is re-examined on its next touch. Adding an extension: the files concerned carry a real embed_sha and no stamp can speak for them, so pruneUnembeddableChunks (src/embed_backfill.ts) sweeps the whole index for chunk rows the current gates would now reject, keyed on tg_version so it reruns exactly once per release, and restamps each pruned file with that gate's own marker so the removal direction above covers those files too rather than leaving them on a bare sha no condition can ever re-open. Hashing this file into EMBED_FINGERPRINT would also work and was rejected on measurement: it re-embeds every file on the machine (~45 minutes of inference on one real 243,603-chunk index) to reach the few hundred that changed verdict, where the two mechanisms above reach exactly those files and no others. Covered by tests/embed_asset_and_chunk_gates.test.ts and tests/embed_backfill.test.ts.
  'src/own_lookup.ts': 'an own-property accessor over a plain-object map, reached via config.ts; it decides whether a lookup answers at all, never how content is chunked or embedded',
  'src/asset_extensions.ts': 'carries its own invalidation instead of this digest\'s: the stamped set id re-examines every file the list stops covering, and the tg_version-keyed backfill sweep reaches every file it newly covers -- see the note above this entry',
...
describe('the embed fingerprint covers every source that decides embedding chunk text and boundaries', () => {
  ...
  it('classifies every closure member as hashed by embedFingerprintSources(), transitively covered by PARSER_FINGERPRINT, or explicitly exempted with a reason', () => {
    ...
    // A file PARSER_FINGERPRINT already hashes forces a reparse of the affected files on any change, and the only route by which a parse-side edit can change what gets embedded is the symbol rows buildEmbeddingBoundaries cuts a file's chunks on. writeParseResult compares that boundary set across every reparse (embeddingBoundariesMoved in src/parser.ts) and drops the file's carried embed_sha when it moved, so the file is re-embedded per-file and exactly when its own cuts changed -- no separate entry in embedFingerprintSources() needed, and no global digest bump, which would re-embed every already-embedded file on the machine. Deliberately NOT the old reason given here, that embedUnchanged requires parseUnchanged: the two gates were decoupled for cause (conjoining them re-embedded the whole index on every parser-stamp bump -- see the comment on embedUnchanged in src/cli.ts), which left this bucket resting on a mechanism that no longer existed.
