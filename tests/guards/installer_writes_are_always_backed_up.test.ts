/** Every file the installer writes, modifies, or appends to must get a timestamped backup via `backupFile` (the same mechanism `writeJsonSettings` already used for `settings.json`) before this session -- `upsertDelimitedBlock`/`stripDelimitedBlock` did not (fixed in `src/util.ts`, see `tests/util.test.ts`), and neither did the direct `atomicWriteText`/`writeFileSync`/ `writeIfDifferent` merge-writers in `src/bridges/cursor_install.ts`, `visualstudio_install.ts`, `vscode_install.ts`, and `zed_install.ts` (their `mcp.json`/`settings.json` writers), nor `src/install.ts`'s `installSkill` (`SKILL.md`) nor `src/bridges/kimi_install.ts`'s skill writer. Real incident this guards: `~/.claude/CLAUDE.md`, a user's own hand-maintained file, was silently rewritten by `install`/`uninstall` with no recovery copy. At scale (hundreds of developers each running `token-goat install`) that is not one recoverable mistake, it is unrecoverable data loss repeated at every seat. "No exceptions, no per-file allowlist of important files" is the standing rule this guard enforces: every write site in the installer is enumerated, and any new site that writes to an existing file without going through a backup path fails on arrival, rather than quietly joining an unbacked set someone has to notice by hand. This is a structural guard (source text only, no I/O beyond reading this repo's own files) with a pinned, non-empty population. Three ways a write site is judged "backed up": 1. It calls `writeJsonSettings` -- that helper always calls `backupFile` first (`src/util.ts`). 2. It calls `upsertDelimitedBlock`/`stripDelimitedBlock` -- both call `backupFile` first, or (for `stripDelimitedBlock`) `backupFile` immediately before the final write (`src/util.ts`). 3. A `backupFile(...)` call, or a `writeIfDifferent(..., true)` call (its optional third argument, literally `true`), textually precedes it inside the same enclosing function body. A write site not backed up by one of those three is a real gap unless it is named in `EXEMPT` with a reason a reviewer can check independently of this guard's own author. */
...
/** Sites this guard would otherwise flag, each with a reason a reviewer can check independently: exactly the "no silent allowlist" rule the task calls for -- an exemption here is a named, inspectable decision, not a gap nobody had to justify. - Generated shim/plugin scripts: pure invocation wrappers (`@echo off` + a `node ... mcp-serve` command line, or an equivalent fixed script constant) regenerated byte-for-byte from the current build on every install. Nothing user-authored is ever in them, so a backup protects no content a re-run could not reproduce identically. - Internal bookkeeping files: the created-configs/backups ledger itself and the Copilot-CLI hooks-owners file are token-goat's own state, never hand-edited by a user, and read only by token-goat. `created_configs.ts`'s own backupFile call (inside `writeLedger`, which is not one of these sites since it is not itself an installer write) would be circular for the ledger to back up itself. - `vscode_install.ts`'s frontmatter write only ever fires when the target file does not exist yet (guarded by `!fs.existsSync(filePath)` immediately above it) -- there is nothing on disk to lose, and `backupFile` itself no-ops on a missing path for the same reason. */
const EXEMPT: ReadonlyArray<{ file: string; fn: string; call: string; reason: string }> = [
  ...
  { file: 'src/bridges/copilot_cli_install.ts', fn: 'installCopilotHooksFile', call: 'writeIfDifferent(legacyScriptPath, HOOKS_SCRIPT_FORWARDER)', reason: 'generated backward-compat forwarder script, same class as installHooks above' },
  ...
  { file: 'src/bridges/grok_install.ts', fn: 'installGrok', call: "writeIfDifferent(grokLegacyHookScriptPath(), legacyShimForwarder('{}'))", reason: 'generated backward-compat forwarder script, same class as the Claude Code forwarder above' },
  ...
  { file: 'src/bridges/kimi_install.ts', fn: 'installKimi', call: "atomicWriteText(kimiLegacyHookScriptPath(), legacyShimForwarder(''))", reason: 'generated backward-compat forwarder script, same class as the Claude Code forwarder above' },
