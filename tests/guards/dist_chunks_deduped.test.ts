 ...
 *
 * A further 12 KB was bought by the hint-efficacy measurement batch. hint_stats.ts, hooks_bash.ts, hooks_common.ts and db.ts are all hook-eager, and the batch touches every one of them: hint builders now hand their own file path to the efficacy ledger instead of it being regex-scraped back out of the rendered hint (a `correlators` field on the context HookOutput, a `pathHint` wrapper and 20 converted call sites in hooks_bash.ts); a range hint must now price its own proposal against the window the command asked for before it may emit, which pulls bash_range_savings.ts and line_regions.ts onto the hook path; and two `hint_emissions` columns, with their migrations, separate an emission that was shown but unscoreable from a detection that was never shown at all. Measured by building both trees on one machine, ceiling temporarily forced to 1 to read the figure at the parent: 2,385,404 bytes over 19 chunks at c6968099 -- which had 516 bytes of headroom, so the first step of the batch alone was going to find this red -- against 2,395,898 with all of it, +10,494 over the batch. Rebuilt twice at that final state, byte-identical each time. The chunk count never moved off 19, so none of this is a new chunk being dragged eager: it is compiled bytes on modules the hook path already loaded. 12 KB leaves 2,310 bytes of headroom, in the same range every paragraph above deliberately keeps.
 */
const MAX_HOOK_EAGER_BYTES = 2.25 * 1024 * 1024 + 4 * 1024 + 2 * 1024 + 3 * 1024 + 1 * 1024 + 1 * 1024 + 1 * 1024 + 3 * 1024 + 4 * 1024 + 1 * 1024 + 6 * 1024 + 12 * 1024
