/** Structural guard on what `dist/` *ships*, as opposed to what it loads. The CLI entry (`token-goat.core.mjs`) and the in-process hook library (`token-goat-hook.mjs`) are two entry points over almost the same set of modules. They were built by two separate `esbuild.build()` calls writing into the same directory under disjoint chunk-name prefixes, which meant esbuild had no way to share anything between them: every module both entries reach was emitted twice. `dist/` was 6.75 MB across 31 files with 3.37 MB of it a verbatim second copy, and all of it shipped, because package.json's `files` takes the whole directory. Building both entries in one call lets code splitting share the chunks: 18 files, 3.37 MB, and the packed tarball 1.60 MB -> 0.84 MB. Nothing failed while the copies were there. Both bundles worked, both loaded the same bytes at startup, and every existing guard passed -- the only symptom was that an install downloaded twice what it needed. That is precisely the shape a size assertion catches and a behavioural one does not, so this file asserts on the emitted file set. The three assertions are deliberately different questions. Duplicate content catches the regression directly. Shared chunks keep that from passing vacuously: deleting one entry entirely would also remove every duplicate. The hook's eager ceiling covers the risk the merge itself introduced, since sharing chunks across entries re-draws every chunk boundary and could have pulled bytes the hook only needs lazily into the set it parses on every tool call. */
...
/** The language modules the hook path may compile. Everything else under src/languages/ is an extractor only the indexer calls, and adding one must not grow the eager set -- which is what every adapter batch did before languages/registry.ts existed. Each name here earns its place by being called on the hook path itself. */
...
/** The `src/...` module markers esbuild writes above each module it inlines, read out of `files`. CAPTURE: the names come from the built bundle itself, not from a list of what we expect to be in it. */
...
function chunkFiles(): string[] {
  // Pinned: the dedup check is a comparison across chunks, so an empty or single-element chunk list has nothing to compare and reports "no duplicates" for the same reason an unbuilt tree would.
  return [
    ...pinnedPopulation({
...
describe('dist chunks are shared, not duplicated', () => {
  it('emits no two chunks with the same content', () => {
    // Sibling specifiers are normalized away first: two copies of one module differ only in which chunk names they point at, so comparing raw bytes would miss exactly the case that matters. Before the merge this found 8 pairs; the prefixes differed but the code did not.
  ...
  it('shares chunks between the two entries rather than giving each its own set', () => {
    // The non-vacuous half. Zero duplicates is also what a dist/ holding only one entry's output looks like, so require that the two entries genuinely reach common chunks.
  ...
  it('reaches a regex adapter only through a deferred chunk', () => {
    // fortran.ts stands for the adapters as a group: it is called from ADAPTER_EXTRACTORS alone, so the hook must be able to reach it (the CLI and worker share these chunks) without compiling it up front.
    const eager = [...closure(HOOK_BUNDLE, 'static')]
  ...
  it('defers at least one chunk from the hook entry', () => {
    // Sharing chunks with the CLI entry could have made every chunk statically reachable from the hook, which is what losing splitting looks like from this side.
  ...
  it('leaves the hook entry free of load-time side effects', () => {
    // The CLI entry calls run() at import time to parse process.argv. Sharing chunks between the two entries is only safe because src/main.ts is reachable from the core entry alone, so that call cannot land in a shared chunk -- assert it by importing the hook bundle with argv the CLI would reject and requiring a silent, successful exit.
