 ...
 *
 * A further 8 KB was bought by the hint-efficacy measurement batch. hint_stats.ts, hooks_bash.ts, hooks_common.ts and config.ts are all hook-eager, and the batch touches every one of them: hint builders now hand their own file path to the efficacy ledger instead of it being regex-scraped back out of the rendered hint (a `correlators` field on the context HookOutput, a `pathHint` wrapper and 20 converted call sites in hooks_bash.ts), a line-count floor with its own config key silences range hints on windows already small enough to be surgical, two `hint_emissions` columns separate a detected hint from a displayed one and an observable emission from an uncreditable one, and the whole-file deny carries a runnable command. Measured by building both trees on one machine: 2,385,404 bytes over 19 chunks at the parent commit c6968099 -- which had 516 bytes of headroom, so the first step of the batch alone was going to find this red -- against 2,386,390 with the correlator-passing step, +986 for that step. 8 KB rather than 2 because the four steps that follow it each add eager rows of exactly the kinds already priced above (one config key at ~0.5 KB per the `hooks.latency_budget_ms` entry, two schema/migration rows, and two new aggregate columns), and discovering this ceiling red four more times in one batch is the failure mode these entries exist to prevent. The chunk count did not move for the step measured here; the batch's final figure is recorded by the last commit in it.
 */
const MAX_HOOK_EAGER_BYTES = 2.25 * 1024 * 1024 + 4 * 1024 + 2 * 1024 + 3 * 1024 + 1 * 1024 + 1 * 1024 + 1 * 1024 + 3 * 1024 + 4 * 1024 + 1 * 1024 + 6 * 1024 + 8 * 1024
