/** Structural guard on what the core bundle *loads* at startup. Every CLI call and every spawned hook process reads `dist/token-goat.mjs`, which imports `dist/token-goat.core.mjs`. V8 compiles a module in full before running any of it, so code sitting behind a dynamic import is still parsed on every single invocation when esbuild inlines it into one file: only its *execution* is deferred. Building with `splitting: true` moves those bytes into sibling chunks that are read only if the dynamic import actually fires. The assertion is on the static import graph of the built output, not on total bundle size: dropping `splitting: true` leaves the total unchanged while moving every byte back into the eager set, which is exactly the regression this guards. Reading dist/ rather than re-bundling means it checks the artifact that actually ships. */
...
/** Ceiling on what the entry may pull in statically. The split build loads about 2.83 MB of a 3.41 MB output; the pre-split monolith was 3.61 MB in one file. The headroom is deliberate -- this is a regression trip-wire for the whole bundle collapsing back into the eager set, not a budget to be tuned on every dependency change. 8 KB was added when origin/main's doctor work merged in: findTopIndexedProjects, the age window on checkUnmappedTools, checkVscodeProjectMcp and cleanupDeprecatedVscodeProjectMcp, alongside the local oversized-db category breakdown that now ships beside it rather than instead of it. Measured 3,411,459 bytes after the merge against the 3,407,872-byte line, which it missed by 3,587. The remaining 4,605 bytes of headroom are deliberate: this is still the collapse trip-wire, and a ceiling raised to within a few hundred bytes of the measurement turns the next unrelated change into a red guard rather than a decision. 4 KB more was added by the per-kind embedding stamp: src/embed_stamp.ts (which extraction kind a path belongs to) plus the kind-scoped reset in src/embeddings.ts and the generated per-kind digest map, all of them on the already-eager embedding path. Measured 3,416,952 bytes against the 3,416,064-byte line, which it missed by 888. The remaining 3,208 bytes of headroom are deliberate, for the same reason the paragraph above gives: the line moves by a measured amount when a feature lands on the eager path, never to whatever the current build happens to weigh. 8 KB more was added by the `answer` question router: src/answer_router.ts, registered from src/cli_cmd_analysis.ts, which is already eager. It pulls in no new chunk -- the eager closure stayed at 31 chunks across all three measurements below -- so the whole cost is the router's own compiled bytes. Three trees measured with the same closure walk this file performs, rebuilt between each: HEAD 3,417,370 bytes; HEAD plus the one-word `--help` group line 3,417,378 (+8); the router wired up 3,424,850 (+7,480 over HEAD), which missed the 3,420,160-byte line by 4,690. The remaining 3,502 bytes of headroom are deliberate, for the same reason both paragraphs above give. 4 KB more was added by bounding the `answer` router's delegates and disclosing what they withhold: the router's shared row cap, the `callers` text-mode truncation notice (which pulls `countRefs`, already exported from the eager `index_reader.ts`), and `symbol --exclude-vendored`. It pulls in no new chunk -- the eager closure stayed at 31 chunks across all four measurements below -- so the whole cost is compiled bytes on the already-eager path. Four trees measured with the same closure walk this file performs, rebuilt between each: HEAD 3,427,946 bytes; plus the router bound 3,428,161 (+215); plus the callers disclosure 3,428,470 (+309); plus `--exclude-vendored` 3,428,908 (+438), which missed the 3,428,352-byte line by 556. The remaining 3,540 bytes of headroom are deliberate, for the same reason every paragraph above gives. 4 KB more was added by the two embedding gates -- src/asset_extensions.ts (the shared non-text extension list, a new leaf module) and src/embed_backfill.ts (the version-keyed sweep, wired into cmdIndex) -- plus the `indexing.max_chunks_per_file` config key. No new chunk: the eager closure stayed at 31 chunks across every measurement, so the whole cost is compiled bytes on the already-eager path. Three trees measured with the same closure walk this file performs, rebuilt between each: HEAD 3,428,908 bytes (the figure the paragraph above records); the gates and the shared extension module alone came in under the 3,432,448-byte line and did not trip this guard; both, with the backfill sweep wired into cmdIndex, 3,433,842 (+4,934 over HEAD), which missed that line by 1,394. Rebuilt four times at that final state, byte-identical each time. The remaining 2,702 bytes of headroom are deliberate, for the same reason every paragraph above gives. 12 KB more was added by line-number addressing on `read` -- the `file:N` / `file:N-M` spec form and the region resolver behind it (parseColonLineSpec, resolveLineRegions, runLineRegion in src/read_spec.ts, wired into runRead and into mcp_server.ts's specFilePart confinement gate). No new chunk and no new module: the eager closure stayed at 31 chunks across both measurements, so the whole cost is compiled bytes on the already-eager read path. Two trees measured with the same closure walk this file performs, rebuilt between each: HEAD 3,434,874 bytes; the line-region path 3,440,241 (+5,367 over HEAD), which overran the previous 3,436,544-byte line by 3,697. Rebuilt three times at that final state, byte-identical each time. An 8 KB bump would have cleared it by only 399 bytes, well under the 2.7-3.5 KB of slack every paragraph above deliberately kept, so this takes 12 KB and leaves 4,495 bytes of headroom, for the same reason those paragraphs give. 8 KB more was bought by the hint-efficacy measurement batch, for the same code the hook-eager paragraph in tests/guards/dist_chunks_deduped.test.ts describes -- the range hint's own net-benefit gate (bash_range_savings.ts, line_regions.ts), the correlator field threaded through hooks_common.ts, and two hint_emissions columns with their migrations in db.ts. No new module and no new chunk: the eager closure held at the same count across both measurements, so the whole cost is compiled bytes on already-eager modules. Two trees measured with the same closure walk this file performs, ceiling temporarily forced to 1 to read the figure at the parent: c6968099 3,440,583 bytes; the batch 3,447,687 (+7,104), which overran the previous 3,444,736-byte line by 2,951. Rebuilt twice at that final state, byte-identical each time. A 4 KB bump would have cleared it by only 1,145 bytes, under the slack every paragraph above keeps, so this takes 8 KB and leaves 5,241 bytes of headroom. 4 KB more was bought by three `semantic` and hint changes: the net-benefit decline reason recorded on every hint that is withheld rather than dropped silently, the fusion rank and retrieval leg carried through to `semantic`'s own output so the number it prints is the number it ranked on, and the `semantic.max_distance` relevance floor with its three config registration sites and its entry in the project-lock list. No new module and no new chunk: the eager closure held at 31 chunks across both measurements, so the whole cost is compiled bytes on already-eager modules. Two trees measured with the same closure walk this file performs, built in separate worktrees off a shared node_modules so neither measurement disturbed the other: HEAD a02ffb02 3,451,345 bytes; the batch 3,453,826 (+2,481), which overran the previous 3,452,928-byte line by 898. Rebuilt three times at that final state, byte-identical each time. This takes 4 KB and leaves 3,198 bytes of headroom, inside the 2.7-5.2 KB slack every paragraph above keeps and for the reason they give. 16 KB more was bought by the compaction-measurement batch: src/manifest.ts (the manifest builder and its printed-path sample, lifted out of compact.ts so the pre_compact hook and the post_compact survival canary cannot drift apart), src/token_estimate.ts (content-class token pricing), src/harness_channels.ts (which harnesses discard a pre_compact return), the post-compaction recovery path in hooks_session_start.ts, web pages in resume.ts's packet, and the estimator calibration in session_audit.ts. Unlike every paragraph above, this one moved the closure itself: 31 chunks before, 35 now, because splitting the manifest out of compact.ts re-partitioned what the eager entry reaches. Measured with the same closure walk this file performs: the previous paragraph's final state 3,453,826 bytes; this batch 3,467,084 (+13,258), which overran the previous 3,457,024-byte line by 10,060. A 12 KB bump would have cleared it by 2,228 bytes, under the 2.7 KB floor every paragraph above keeps, so this takes 16 KB and leaves 6,324 bytes of headroom -- slightly more than the 5.2 KB any single paragraph has kept before, deliberately, because a re-partitioned closure is the one case where the next unrelated change can move by more than its own compiled size. 1 KB more was bought by the batch that gave the Copilot CLI shim own-property lookups and the compaction manifest its subagents' web fetches. Both files are eager: the shim script is a string constant on the bridge registry's graph, and manifest.ts is what pre_compact builds. No new module and no new chunk -- the closure held across both measurements, so the whole cost is compiled bytes on already-eager modules, most of it the shim's own comment text, which ships verbatim inside the template literal rather than being stripped like source comments are. Measured with the same closure walk this file performs, ceiling temporarily forced to 1 to read the figure at the parent, and with the working tree restored to HEAD for the parent build rather than stashed: 3d1a799e 3,472,466 bytes; the batch 3,473,379 (+913), which cleared the previous 3,473,408-byte line by 29. Rebuilt at that final state, byte-identical. 29 bytes is not headroom -- it is the next one-line change discovering a red guard -- so this takes 1 KB and leaves 1,053 bytes. A further 2 KB was bought by the batch that taught the launcher prefix stripper to skip a launcher's own options and the yarn script resolver to leave a built-in alone. helpers.ts is eager through the bash filter registry, and the whole cost is two string-literal lookup sets on it -- LAUNCHER_VALUE_FLAGS, the flags that consume the token after them, and YARN_BUILTIN_SUBCOMMANDS, the 47 names a bare `yarn <name>` runs as a built-in rather than as a script. No new module and no new chunk: the closure held across both measurements, so this is compiled bytes on an already-eager module. Measured with the same closure walk this file performs, ceiling temporarily forced to 1 to read the figure at the parent, and with the working tree restored to HEAD for the parent build rather than stashed: 558b1c96 3,473,299 bytes; the batch 3,474,799 (+1,500), which overran the previous 3,474,432-byte line by 367. Rebuilt twice at each state, byte-identical both times. A 1 KB bump would clear it by 657 bytes, under every margin the paragraphs above keep, so this takes 2 KB and leaves 1,681 bytes. * A further 2 KB was bought by the batch that reclaims vectors whose chunk row is already gone and bounds semantic.max_distance to the scan's own threshold. index_prune.ts and config.ts are both eager, and the whole cost is compiled bytes on them -- no new module, no new import edge, so the closure held across both measurements: pruneOrphanedVectors pulls in nothing index_prune.ts did not already have, and config.ts's change is a bound and the comment explaining it. Most of the figure is comment text, which this build does not strip. Measured with the same closure walk this file performs, ceiling temporarily forced to 1 to read the figure at the parent, and with the working tree restored to HEAD for the parent build rather than stashed: 17f30b23 3,475,236 bytes; the batch 3,476,643 (+1,407), which overran the previous 3,476,480-byte line by 163. A 1 KB bump would clear it by 861 bytes, under the 1,053 the tightest paragraph above keeps, so this takes 2 KB and leaves 1,885 bytes. 8 KB more was bought by automatic Copilot CLI session detection in `token-goat waste`: scanning session-state, active process/lock inspection, workspace.yaml project-matching, and format auto-routing in src/copilot_waste.ts and src/cli_waste.ts. No new module and no new chunk: the eager closure held at 31 chunks across both measurements, so the cost is compiled bytes on already-eager modules. Measured 3,460,945 bytes against the 3,457,024-byte line, which it missed by 3,921. This takes 8 KB and leaves 4,271 bytes of headroom, inside the 2.7-5.2 KB slack every paragraph above keeps and for the reason they give. 8 KB more was bought by first-class `token-goat audit` CLI command registration and Maintainer Feedback Card generation in src/cli_audit.ts, src/cli_session.ts, and src/cli_cmd_session.ts. No new chunk: the eager closure held at 31 chunks across both measurements, so the cost is compiled bytes on already-eager modules. Measured 3,467,060 bytes against the 3,465,216-byte line, which it missed by 1,844. This takes 8 KB and leaves 6,348 bytes of headroom, inside the slack every paragraph above keeps and for the reason they give. 24 KB more was bought by workflow enhancements: parameter-aware PowerShell head allowance (Get-Content -TotalCount/-First/-Head) in src/bash_extractors.ts and src/hooks_bash.ts, heading slicing (--max-lines/--head) in src/read_section.ts, merge conflict context lines in src/conflict_query.ts and src/read_git.ts, and configurable index size warning threshold (indexing.max_db_size_mb) with auto-reclaim in src/cli_doctor.ts, src/index_reclaim.ts, and src/config.ts. No new chunk: the eager closure held at 31 chunks, so the cost is compiled bytes on already-eager modules. Measured 3,490,531 bytes against the 3,473,408-byte line, which it missed by 17,123. This takes 24 KB and leaves 7,453 bytes of headroom, inside the slack every paragraph above keeps and for the reason they give. 8 KB more was bought by Claude Code's PostToolUseFailure wiring: `missingHookEvents` in src/install.ts and the `doctor` check that reports an install predating an event, the `--window-days` and `--tool-errors` options on `session-audit`, and the tool_failure stat kind. The census and the classifier it runs (src/tool_error_census.ts, src/tool_error_class.ts, and through the classifier bash_extractors.ts) are reached only through a dynamic import from session_audit.ts and cli_session.ts, because a static import put 66,290 bytes on this set; the five new chunks, 39 to 44, are that split's boundaries. Measured by swapping the HEAD versions of the changed sources into this checkout and building: HEAD 3,516,691 bytes, the batch 3,524,927 (+8,236), overrunning the previous 3,519,488-byte line by 5,439. This takes 8 KB and leaves 2,753. 2 KB more was bought by `semantic` taking several queries in one call (runSemanticMulti in src/read_commands.ts and cmdSemantic's wiring in src/cli.ts) and its weak-closest-match notice, all compiled bytes on the already-eager read path; source comments do not ship, so folding read_commands.ts's comments in the same change moved nothing. No new chunk: the closure held at 44 across both measurements. Measured by swapping the HEAD versions of the changed sources into this checkout and building: HEAD 55796265 3,526,074 bytes, the change 3,527,817 (+1,743), overrunning the previous 3,527,680-byte line by 137; rebuilt twice at that state, byte-identical. A 1 KB bump would clear it by only 887 bytes, under the margins kept above, so this takes 2 KB and leaves 1,911. A further 14 KB is src/hint_target.ts, the resolver that puts a real heading, key, table or symbol into every deny and read hint and shortens a repeated deny, reached through hooks_read.ts on the already-eager read path with no new chunk (44 before and after): HEAD e5ca3a21 3,527,817 bytes, the change 3,541,358 (+13,541), overrunning the previous 3,529,728-byte line by 11,630, so this takes 14 KB and leaves 2,706. A further 4 KB is the read-only index fallback: db.ts's read-only and immutable opens with their error classifier and schema check, sqlite_driver.ts's immutable option, and the read-command gate in cli.ts, with no new chunk (44 before and after): HEAD 43b59523 3,542,034 bytes, the other uncommitted changes of the same batch 3,542,307 (both from worktree builds less their constant 1,824-byte path offset), the change 3,547,022 (+4,715), overrunning the previous 3,544,064-byte line by 2,958, so this takes 4 KB and leaves 1,138. A further 10 KB is doctor measuring each table's pages with its indexes and naming the command that shrinks the largest, the embedding-coverage reasons it counts with the indexer's own freshness gate, the worker check for a data directory that refuses writes, the process-list failure reasons, `hint-stats --session-id`, the worker's embed backlog sweep and unwritable-directory refusal, the damaged-database fail-fast in db.ts, command errors printed line by line, and the Bash hook refusing a line range over a whole file the way it refuses `cat`, with no new chunk (44 before and after): HEAD 561feedb 3,547,312 bytes, measured by swapping the HEAD versions of the 14 changed sources into this checkout and building, the change 3,557,621 (+10,309), overrunning the previous 3,548,160-byte line by 9,461, so this takes 10 KB and leaves 779. A further 2 KB is the read commands pricing a search-shaped result at its largest matched file and a shell-shaped one at what the harness would deliver, which makes delivery_cap.ts shared between the command chunks and the hooks, so esbuild splits it into its own 1,338-byte chunk (44 before, 45 after): the rest of the batch 3,557,663 bytes, measured by swapping the HEAD versions of read_commands.ts, read_git.ts and cli_stats.ts into this checkout and building, the change 3,560,157 (+2,494), overrunning the previous 3,558,400-byte line by 1,757, so this takes 2 KB and leaves 291. A further 1 KB is `doctor` and `install` matching a hook entry whose bundle path is another spelling of the same file, in install.ts, with no new chunk (45 before and after): the rest of the batch 3,560,157 bytes, the change 3,560,949 (+792), overrunning the previous 3,560,448-byte line by 501, so this takes 1 KB and leaves 523. A further 1 KB is the Copilot CLI hook shim moving to a .cjs file with a .js forwarder left for running sessions, and doctor inspecting project-scope Copilot hooks (copilot_cli_install.ts and cli_doctor.ts): 3,560,949 bytes before, 3,561,786 after (+837), overrunning the previous 3,561,472-byte line by 314, so this takes 1 KB and leaves 710. A further 1 KB is doctor running a project's Copilot hook command only when this machine wrote that hooks file, detecting an install still on the .js shim, and naming the scope's own repair command (cli_doctor.ts), plus the forwarder answering {} when the .cjs shim cannot load: 3,561,786 bytes before, 3,563,219 after (+1,433), overrunning the previous 3,562,496-byte line by 723, so this takes 1 KB and leaves 301. A further 1 KB is the MCP confinement gate and pack taking a file identity through an open handle rather than a path stat, so it matches the fstat it is compared against on Windows releases whose path stat reports no volume serial (handle_stat.ts, mcp_server.ts, pack.ts): 3,563,219 bytes before, 3,563,722 after (+503), overrunning the previous 3,563,520-byte line by 202, so this takes 1 KB and leaves 822. */
const MAX_EAGER_BYTES = 3.25 * 1024 * 1024 + 24 * 1024 + 12 * 1024 + 8 * 1024 + 4 * 1024 + 16 * 1024 + 1 * 1024 + 2 * 1024 + 2 * 1024 + 8 * 1024 + 8 * 1024 + 24 * 1024 + 8 * 1024 + 2 * 1024 + 14 * 1024 + 4 * 1024 + 10 * 1024 + 2 * 1024 + 1 * 1024 + 1 * 1024 + 1 * 1024 + 1 * 1024
...
function staticChunkImports(file: string): string[] {
  ...
  // esbuild emits static chunk edges as `from"./name.mjs"` or `import"./name.mjs"`; a deferred edge is `import("./name.mjs")` with parentheses, which neither pattern matches.
...
describe('core bundle stays split', () => {
  ...
  it('defers at least one chunk instead of importing them all statically', () => {
    // The non-vacuous half: chunks existing proves nothing on its own if the entry statically imports every one of them, which is what a monolith looks like after a mechanical split. Scoped to the chunks this entry actually reaches, not everything sharing the prefix: one build now emits both entries into the same prefix, so a chunk only the hook library reaches would otherwise count as deferred here and let the assertion pass with the core deferring nothing at all.
  ...
  it('leaves dist/ internally consistent: every chunk an emitted file imports exists on disk', () => {
    // Splitting made a partial dist/ possible for the first time. The stale-chunk sweep used to run before its build, so mid-build the previous entry was still present and still importing chunks that had just been deleted; anything starting the CLI then died with ERR_MODULE_NOT_FOUND. Both static and dynamic edges are checked here, since a dynamic one fails just as hard, only later. Pinned: ENTRY is always in this list, so `files` is never empty and the resolvability sweep below would still look busy after the chunk glob stopped matching anything. The floor is on the chunks specifically, which is the part that can silently go to zero.
...
describe('sweepStaleChunks', () => {
  // Tested directly rather than by asserting dist/ is orphan-free after the fact: the suite's globalSetup rebuilds before any test runs, and on a fresh checkout that build starts from an empty dist/, so an end-state assertion there passes even with the sweep deleted outright.
  let dir: string
  ...
  it('never touches a differently-prefixed chunk or the entry files', () => {
    // Prefix scoping is what lets one build sweep its own chunks and, in a second call, clear the legacy token-goat-hook-chunk- set left behind by the era of two separate builds. A sweep matching too broadly would take the entry files with it, and unlike a stale chunk, nothing recreates those until the next build.
  ...
  it('an empty prefix clears an orphaned entry file from a retired name, but never a non-.mjs file', () => {
    // A prior entry-point name (e.g. before ENTRY_POINTS was renamed) leaves its output behind forever: unlike a chunk, nothing re-emits it under a new content hash, and package.json's `files: ["dist/"]` ships it in the tarball. Widening the prefix to '' is how the build catches that case too, so it must not also catch a tracked non-build file like dist/.npmignore that merely happens to sit in the same directory.
    write('_m.mjs')
