 ...
 *
 * 8 KB more was bought by the hint-efficacy measurement batch, for the same code the hook-eager paragraph in tests/guards/dist_chunks_deduped.test.ts describes -- the range hint's own net-benefit gate (bash_range_savings.ts, line_regions.ts), the correlator field threaded through hooks_common.ts, and two hint_emissions columns with their migrations in db.ts. No new module and no new chunk: the eager closure held at the same count across both measurements, so the whole cost is compiled bytes on already-eager modules. Two trees measured with the same closure walk this file performs, ceiling temporarily forced to 1 to read the figure at the parent: c6968099 3,440,583 bytes; the batch 3,447,669 (+7,086), which overran the previous 3,444,736-byte line by 2,933. Rebuilt twice at that final state, byte-identical each time. A 4 KB bump would have cleared it by only 1,163 bytes, under the slack every paragraph above keeps, so this takes 8 KB and leaves 5,259 bytes of headroom.
 */
const MAX_EAGER_BYTES = 3.25 * 1024 * 1024 + 24 * 1024 + 12 * 1024 + 8 * 1024
