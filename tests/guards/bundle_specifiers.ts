...
/**
 * The syntactic positions a module specifier can occupy in esbuild's output, as regex source with `SPEC` standing in for the specifier pattern the caller wants.
 *
 * The last entry covers the `createRequire(import.meta.url)('jsonc-parser')` shape, where the specifier follows a call rather than the `require` keyword. It is anchored on `createRequire` rather than on a bare `)(`, which in a bundle full of IIFEs would match almost anything.
 */
const FORMS = [
  '\\bfrom\\s*SPEC',
  '\\bimport\\s*\\(\\s*SPEC',
 ...
 *
 * Matches import/require *syntax* rather than the bare name, because the bundle also contains the string `vendor: "zod"` inside inlined code, which is data and not a resolution.
 */
...
/**
 * Every specifier the bundle resolves, whatever it names -- the same match, read forwards.
 *
 * `resolvesSpecifier` answers "is this one package resolved", which can only ever check names someone already thought to list. This returns the set, so a package nobody thought of is still seen.
 */
...
/**
 * Assert the matcher still matches, using a specifier the bundle is known to resolve.
 *
 * Without this every "is not resolved" assertion in either guard passes the moment the matcher stops matching anything, which is the failure mode that leaves a supply-chain guard green over an unguarded bundle.
 */
