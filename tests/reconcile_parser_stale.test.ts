/**
 * `token-goat reconcile` must find rows a parser upgrade left stale, not only rows whose content moved.
 *
 * `reconcileProject`'s drift sweep used to compare content (mtime, then sha) only. A parser change (new syntax extracted, a fixed extraction bug) invalidates every already-indexed file whose BYTES never moved but whose symbol/ref rows were written by the old extractor -- `files.sha` cannot see that, because it never changes. `files.parser_sha` exists to answer exactly this question (see parser_fingerprint_gate.test.ts), and per-file freshness gates in worker.ts/cli.ts already consult it -- but nothing FOUND those rows for a file nobody edits after the bump. This is the same failure shape as an earlier fix in this repo ("a freshness gate that fails closed and queues nothing misses forever", content-only key left 37/237 stale): documented once, and it recurred on the parse side because the fix was never enforced by a guard.
 *
 * Provenance: CAPTURE. The stale fingerprint is written directly into the real indexed database (the shape a binary upgrade leaves: same bytes on disk, an old parser_sha on the row), then the real built bundle's `reconcile` command is run against it and its actual stdout/JSON is asserted on -- this drives the true default path end to end (index -> stale row -> reconcile -> enqueued
...
describe('reconcile against a parser upgrade', () => {
  it('calibration: a real symbol resolves before the fixture is made stale, so the cases below prove something', () => {
    ...
    // Calibration for the parser-only cases below: a content-only diff must find nothing on a freshly indexed project. Checked BEFORE any parser_sha is stamped stale, or a sweep that already flagged everything as changed would make every later case pass for the wrong reason.
  ...
  it('finds a file whose content never moved but whose rows carry an old parser fingerprint', () => {
    ...
    // getDb, not a bare sqlite handle: the schema's triggers call the TG_LOWER custom function getDb registers on the connection it opens, and a raw connection that skips that registration fails on the very first DML against a path-indexed table.
  ...
  it('end to end: drains the queue with the real default indexer, reparses with the current parser, and the row converges', () => {
    ...
    // Drop the symbol row too, the same "before" shape parser_fingerprint_gate.test.ts uses: a stale files.parser_sha with symbols still sitting from the old parse would let this test pass even if drainOnce never actually reparsed anything.
    ...
    // Drive the REAL default worker drain path: drainOnce with no injected index/remove callback resolves to makeIndexer(dbPath)/makeRemover(dbPath), the exact production default the detached daemon's own loop uses (worker.ts::runWorkerLoop) -- this is deliberately NOT a test-supplied callback, which is the injected-seam trap CLAUDE.md's critical-path note warns this exact pipeline has shipped broken behind before. `dir` is derived from the queue file drainOnce itself defines the layout for (dir/queue/dirty.txt, dir/global.db), not re-derived from dataDir()/env: this test process's own dataDir() is fixed at import time by tests/setup/isolate-home.ts and would not point at this fixture's isolated home dir.
    ...
    // The actual point of the fix: symbols were dropped above, and only a real reparse rebuilds them -- querying the exact fixture DB, not a fresh one, so this can only pass if drainOnce truly reparsed the file rather than merely touching the files row.
