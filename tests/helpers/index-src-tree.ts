 ...
 *
 * The bound is here to catch a hook that never finishes, not to budget its work, so it is left far above what the work costs: 300s against 4.6s is roughly 65x. It has now fired twice anyway, both on windows-latest shard 1 and both in `graph_commands` and `json_envelope_shape` at once: run 35852365360 (2026-09-23) and run 35864968435 (2026-09-23, commit e52494d3). Each time an immediate re-run of the identical commit passed, 13/13 jobs green, with nothing rebuilt and no source touched. Two occurrences with the same signature and a clean re-run on both is a stalled runner, not work that has grown into the bound, which is why the number still has not moved -- raising it would only lengthen the stall before the same runner gives up. Note also that a re-run overwrites the conclusion `gh run list` reports, so neither failure is visible in that history: the occurrences are recorded here because there is nowhere else they survive. Raise this only when a fresh measurement of the wall time above says the work itself has grown into it.
 */
