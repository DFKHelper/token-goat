...
// A TOKEN_GOAT_HOME inherited from the ENVIRONMENT (a developer or CI exporting one) must still win. A TOKEN_GOAT_HOME left in process.env by a PREVIOUS test file in this same reused fork must not -- that is the leak. The sentinel distinguishes the two, which a bare presence check cannot. Both directories below used to be created straight in os.tmpdir() and removed from a process.on("exit") handler. Vitest kills its workers instead of letting them exit, so that handler almost never fires and they accumulated without bound -- 946,101 of the 1,407,592 entries in this machine's %TEMP% were these two prefixes. globalSetup (build-bundle.ts) now creates one root per run and deletes it from the main process, which does exit normally, so anything left behind here goes with it. The exit handlers stay: when they do fire they keep the run root small while the run is still going.
function runRoot(): string {
  return process.env['TG_TEST_RUN_ROOT'] ?? os.tmpdir()
}

// Point the process-wide temp dir at that same run root. tests/ has ~981 `os.tmpdir()` call sites across 272 files, and every one that mkdtemps a fixture dir used to leave it in the real %TEMP% forever: vitest kills its workers, so per-test `process.on('exit')` cleanup almost never fires. One full suite run leaked ~375 entries that way, and the accumulated backlog is not a cosmetic problem -- at 1.45M entries NTFS directory-lock contention made every `mkdtemp` in %TEMP% cost 170 ms instead of 0 ms, which took the suite from 66 s to 378 s and timed out a test doing nothing but two synchronous calls.
//
// Node re-reads TEMP/TMP/TMPDIR on each `os.tmpdir()` call rather than caching, so pinning them here moves all 981 sites under the directory globalSetup already deletes on teardown, with no call-site changes. It also covers the ~975 CLI child processes, which inherit this env, and any temp a dependency creates on its own. Only set when there IS a run root: without one `runRoot()` returns `os.tmpdir()` and this would be a self-assignment.
...
// APPDATA and XDG_CONFIG_HOME are the CONFIG-roots, and redirecting only the DATA-roots above left a live hole in exactly the tests most able to do damage with it. Three bridges read `process.env['APPDATA']` directly on Windows -- vscode_install, zed_install, opencode_install -- so an install probe spawned by a test wrote `%APPDATA%\Zed\settings.json` and `%APPDATA%\Zed\token-goat-mcp.cmd` into the developer's REAL user profile; that was hit for real during an audit, reverted by hand. The read direction is the subtler half and is why this belongs here rather than in one guard: `install --vscode -p`'s clean control calls `otherScopeHasManagedServer`, which reads the real `%APPDATA%\Code\User\mcp.json`. That file happens not to exist on this machine, so the suite is green -- but on a machine where token-goat IS registered in VS Code user scope the control throws and the guard fails for environmental reasons having nothing to do with the code. Redirected here, once, so every test file and every child process that inherits this env gets it rather than each spawner remembering.
process.env['APPDATA'] = dataHome
process.env['XDG_CONFIG_HOME'] = dataHome
// macOS ignores LOCALAPPDATA/XDG_DATA_HOME and derives Application Support from HOME, so HOME has to be redirected there for DATA_DIR to land in the sandbox at all.
//
// It is redirected on every platform, not just darwin, because HOME/USERPROFILE is also what `os.homedir()` reads -- and code paths that call it directly (waste's transcript fallback to `~/.claude/projects`, bootstrap-audit, the bridge installers, context-stats' CLAUDE.md walk) escaped the sandbox entirely on Windows and Linux while LOCALAPPDATA/XDG_DATA_HOME were redirected. That gap produced a real intermittent failure: the `waste --top` cases read the developer's own live Claude Code session transcript (76 MB and growing during a dogfooding session), spending ~7s per call parsing it, which blew the test timeout under full-suite contention. Sandboxing HOME everywhere closes the class, not just that one instance.
...
//
// A commit made from a linked worktree is worse: git also exports `GIT_DIR`, as that worktree's absolute gitdir, so a scratch-repo test's `git init` reinitializes the real repository instead of creating one in its temp directory. Measured on one pre-commit guard run in a worktree: nine fixture commits landed on the worktree's branch, its index was replaced by fixture files, the shared `.git/config` took the fixtures' `user.name`/`user.email`, and `core.bare = true` left the main checkout refusing every work-tree command. So every variable git itself lists as repository-local goes, not the one that was first seen to leak.
//
