...
describe('isParseSkipEligible', () => {
  ...
  it('honors indexing.skip_files as configurable, not hardcoded: an empty list re-includes coverage.json (opt-out), and a custom basename excludes a file the default list never covered (opt-in)', () => {
    const optedOut = {skip_dirs: [], skip_files: [], large_file_skip_kb: 1, large_file_symbol_only_kb: 1048576, embeddings_enabled: true, cross_project_symbols: true, max_chunks_per_file: 600, max_db_size_mb: 1500, auto_reclaim_embeddings: false}
    expect(isParseSkipEligible('/repo/coverage.json', optedOut)).toBe(false)

    const optedIn = {skip_dirs: [], skip_files: ['lcov.json'], large_file_skip_kb: 1, large_file_symbol_only_kb: 1048576, embeddings_enabled: true, cross_project_symbols: true, max_chunks_per_file: 600, max_db_size_mb: 1500, auto_reclaim_embeddings: false}
