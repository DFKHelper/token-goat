...
describe('truncateLine output length constraint regression', () => {
  it('respects maxLineLength when adding truncation message', () => {
    // Regression test for bug: truncateLine appended "… [N chars truncated]"
    // without accounting for the message length, causing the output to exceed maxLineLength.
    //
    // Example: with maxLineLength=500, a line of 1000 chars would produce:
    //   content[0:500] + "… [500 chars truncated]" = 500 + 23 chars = 523 chars
    // which exceeds the 500 char limit.
