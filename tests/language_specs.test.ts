...
// The brace batch: Objective-C (`.m` and `.h` only by content), Groovy, Perl, Solidity, Thrift and four shader languages.
const BATCH_A_CODE = ['.mm', '.groovy', '.gvy', '.gradle', '.pl', '.pm', '.sol', '.glsl', '.vert', '.frag', '.comp', '.geom', '.tesc', '.tese', '.hlsl', '.hlsli', '.wgsl', '.metal']
const BATCH_A = [...BATCH_A_CODE, '.thrift']
const BATCH_A_IDS = ['objc', 'groovy', 'perl', 'solidity', 'thrift', 'glsl', 'hlsl', 'wgsl', 'metal']
// The keyword-end batch: Fortran, Pascal (with Delphi forms), CMake by extension and CMakeLists.txt by name; MATLAB has no extension of its own and is found by content.
const BATCH_B1 = ['.f', '.for', '.f77', '.f90', '.f95', '.f03', '.f08', '.pas', '.dpr', '.dpk', '.lpr', '.dfm', '.cmake']
const BATCH_B1_IDS = ['fortran', 'pascal', 'matlab', 'cmake']
// The label batch: Assembly (GAS, NASM and IBM HLASM share one adapter), Windows batch files and Erlang.
const BATCH_D = ['.s', '.asm', '.nasm', '.bat', '.cmd', '.erl', '.hrl']
const BATCH_D_IDS = ['asm', 'batch', 'erlang']
...
describe('language table: derived lists match the pre-refactor lists except the named differences', () => {
  it('extension map: only the new mappings were added, nothing moved or dropped', () => {
    ...
    expect(sorted(added)).toEqual(sorted(['.zsh', '.ksh', '.bats', '.bzl', '.star', ...PLSQL, '.jsonc', '.avsc', ...BATCH_C, ...BATCH_A, ...BATCH_B1, ...BATCH_D]))
  ...
  it('bash hook symbol-bearing set is unchanged', () => {
    ...
    expect(sorted(now)).toEqual(sorted([...OLD_SYMBOL_BEARING, ...BATCH_C_IDS, ...BATCH_A_IDS, ...BATCH_B1_IDS, ...BATCH_D_IDS]))
  })

  it('read hook source-hint set: adds the other Ruby extensions and Starlark only', () => {
    expect(diff(oldReadSource, 'sourceHints')).toEqual({ added: sorted(['.bzl', '.rake', '.ruby', '.star', ...BATCH_C, ...BATCH_A_CODE, ...BATCH_B1, ...BATCH_D]), removed: [] })
  })
  ...
  it('diff-on-reread set: adds extensions of languages already covered, and the new mappings', () => {
    expect(diff((e) => OLD_DIFFABLE_SOURCE_RE.test(`x${e}`), 'diffable')).toEqual({
      added: sorted(['.avsc', '.bzl', '.cts', '.hxx', '.kts', '.mts', '.pyi', '.rake', '.ruby', '.star', ...PLSQL, ...BATCH_C, ...BATCH_A_CODE, ...BATCH_B1, ...BATCH_D]),
      removed: [],
    })
  })
  ...
  it('grep hook source set: gains the 18 indexed extensions it missed, drops clj (no Clojure adapter)', () => {
    ...
    expect(diff((e) => OLD_GREP_SOURCE_EXT_RE.test(`x${e}`), 'grepSource')).toEqual({
      added: sorted([...missed18, '.ruby', '.rake', '.bzl', '.star', ...SF_MARKUP, ...BATCH_C, ...BATCH_A_CODE, ...BATCH_B1, ...BATCH_D]),
      removed: ['.clj'],
    })
  })
  ...
  it('labels: unchanged except Apex, which used to print as the bare id', () => {
    ...
    expect(changed).toEqual(['abap', 'sas', 'pli', 'rpg', 'jcl', ...BATCH_A_IDS, ...BATCH_B1_IDS, ...BATCH_D_IDS, 'abl', 'apex'])
