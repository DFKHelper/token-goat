...
describe('a target that is only in-root after normalization is refused', () => {
  ...
  it.runIf(POSIX)('refuses a WSL mount-form path that normalizes into a relative in-root one', async () => {
    // Calibration first. If normalizePath ever stops rewriting the mount form, or stops producing a relative result on POSIX, this fixture no longer exercises the asymmetry and a pass below would prove nothing.
    ...
    // The assertion is on the REFUSAL, not on leaked bytes, and deliberately so. Building a real file under a real `/mnt/<letter>/` needs root on a CI runner, and a first draft of this test that faked one under a temp directory passed with the fix reverted -- the canary sat at `$TMPDIR/tg-mnt-XXX/mnt/c/secret.ts` while the attack string named the actual `/mnt/c`, so nothing was there to leak and the pass proved nothing. What the gate owes here is independent of whether the file happens to exist: a path that is absolute and outside the root must be refused as outside it, not admitted and then merely fail to open. Those two outcomes are distinguishable in the response text, which is what this asserts.
