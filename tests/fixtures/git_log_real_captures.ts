...
// CAPTURE, 2026-09-17: `git log --stat -12 --no-color` in this repo, 346 lines, 43 stat lines.
export const CAPTURE_STAT_12 = `commit 0b94da9969425eaa34f41012abbafbb2cfd56add
Author: Token Goat Test <test@token-goat.local>
Date:   Thu Sep 17 14:46:16 2026 -0500

    fix(git): route --name-only, --numstat, and --stat=<width> logs as stat shapes
    
    _compressGitLogEnhanced's isStat set named --stat, --shortstat, and
    --name-status, matching --stat only by exact flag equality. --name-only and
    --numstat fell through to the full-format collapser (above 10 commits) or the
    --oneline hash-only branch (at any commit count), both of which discard every
    line that is neither a commit header nor an indented subject -- a bare path or
    a numstat line is neither, so every filename vanished while the log still
    looked complete and cleared the net-benefit floor. --stat=<width> missed the
    set for a different reason: it arrives in argv as a single token, which
    flags.has('--stat') never matches by equality.
    
    All three are now matched into isStat and pass through the per-commit stat
    compressor unchanged, the same as --name-status already did.
    
    Gates: lint clean, typecheck and typecheck:tests clean, full suite 796 files /
    14796 passed / 51 skipped, build + bench 96.1% saved with fidelity 6/6 (no
    movement), test:matrix 164 passed. Mutation: reverting --name-only alone turns
    the --name-only and --oneline --name-only fixtures red while the --numstat
    fixture and the --stat=<width> case stay green; reverting --numstat alone
    turns only that fixture red; reverting the --stat= prefix match alone turns
    only the --stat=<width> case red. The pre-existing 20-commit full-format
    collapse test stays green throughout, confirming the change did not widen the
    collapser's own trigger.

 CHANGELOG.md                            |   2 +
 src/tool_filters/git.ts                 |   8 +-
 tests/fixtures/git_log_real_captures.ts | 692 ++++++++++++++++++++++++++++++++
 tests/tool_filters_git.test.ts          |  85 ++++
 4 files changed, 786 insertions(+), 1 deletion(-)

commit 1a2495725c54231cdc5495885a02801ce85045c3
Author: Token Goat Test <test@token-goat.local>
Date:   Thu Sep 17 14:23:58 2026 -0500
 ...
 CHANGELOG.md                                |   2 +
 src/tool_filters/package_managers.ts        |  23 +----
 tests/tool_filters_package_managers.test.ts | 150 +++++++++++++++++-----------
...
Date:   Thu Sep 17 12:48:20 2026 -0500
 ...
 CHANGELOG.md                             |  2 ++
 src/tool_filters/pytest.ts               |  6 ++++--
 tests/tool_filters_pytest_gotest.test.ts | 26 ++++++++++++++++++++++++++
...
Date:   Thu Sep 17 12:28:36 2026 -0500
 ...
 CHANGELOG.md                   |  2 ++
 src/tool_filters/git.ts        |  9 +++++++--
 tests/tool_filters_git.test.ts | 43 ++++++++++++++++++++++++++++++++++++++++++
...
Date:   Thu Sep 17 12:16:10 2026 -0500
 ...
 CHANGELOG.md               |  2 ++
 src/embed_fingerprint.ts   |  2 +-
 src/xlsx_extract.ts        |  3 +++
 src/xlsx_reader.ts         |  6 ++++--
 tests/xlsx_extract.test.ts | 20 ++++++++++++++++++++
 tests/xlsx_reader.test.ts  | 14 ++++++++++++++
...
Date:   Thu Sep 17 12:04:31 2026 -0500
 ...
 CHANGELOG.md                     |  2 ++
 src/transcript_extract.ts        | 19 +++++++++++++------
 tests/transcript_extract.test.ts | 14 ++++++++++++++
...
Date:   Thu Sep 17 11:50:47 2026 -0500
 ...
 CHANGELOG.md                     | 2 ++
 src/transcript_extract.ts        | 3 ++-
 tests/transcript_extract.test.ts | 9 +++++++++
...
Date:   Thu Sep 17 11:41:28 2026 -0500
 ...
 CHANGELOG.md               |  2 ++
 src/docx_extract.ts        |  6 ++++--
 src/embed_fingerprint.ts   |  2 +-
 src/ooxml_extract.ts       | 17 +++++++++++++++++
 tests/docx_extract.test.ts | 27 +++++++++++++++++++++++++++
...
Date:   Thu Sep 17 11:31:40 2026 -0500
 ...
 CHANGELOG.md               |  2 ++
 src/docx_extract.ts        | 11 ++++++++---
 src/embed_fingerprint.ts   |  2 +-
 tests/docx_extract.test.ts | 48 ++++++++++++++++++++++++++++++++++++++++++++--
...
Date:   Thu Sep 17 11:17:03 2026 -0500
 ...
 CHANGELOG.md                |  2 ++
 src/text_trace.ts           |  5 +++--
 tests/text_commands.test.ts | 37 +++++++++++++++++++++++++++++++++++++
...
Date:   Thu Sep 17 11:06:12 2026 -0500
 ...
 CHANGELOG.md                |  2 ++
 src/text_trace.ts           |  2 +-
 tests/text_commands.test.ts | 23 +++++++++++++++++++++++
...
Date:   Thu Sep 17 10:57:07 2026 -0500
 ...
 CHANGELOG.md                |  2 ++
 src/text_todo.ts            | 20 +++++++++++++-------
 tests/text_commands.test.ts | 17 +++++++++++++++++
