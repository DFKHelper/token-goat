/** Cross-process session-persistence smoke test against the BUILT bundle. The hooks run as a fresh `token-goat hook <event>` OS process per tool call, so session state (re-read dedup, bash-output recall index) only works if it is persisted to disk between processes. Before this layer existed, a second `pre_tool_use` for the same file in a separate process saw a cold session and emitted nothing. These tests drive the real bundle via separate `spawnSync` invocations sharing one TOKEN_GOAT_HOME and session id, asserting state survives the process boundary. They fail on a port without disk persistence. */
...
beforeAll(() => {
  ...
  // TOKEN_GOAT_HOME isolates cross-process session state (disk_cache.ts's tokenGoatHome()), but config.toml lives under a separate path resolved by src/constants.ts's dataDir(), which on Windows honors LOCALAPPDATA instead -- without also isolating that, these spawned processes would read (and this suite would depend on) whatever the real machine's config.toml happens to contain. Pin LOCALAPPDATA/XDG_DATA_HOME to an isolated dir too, and seed protect_recent_reads = 0 there: these tests assert an immediate re-read still hard-denies, the exact scenario hints.protect_recent_reads's default (4) is designed to exempt (see tests/hooks_read.test.ts for that field's own dedicated coverage).
...
describe('markdown hard re-read deny survives an intervening unrelated tool call', () => {
  it('Read -> PostToolUse -> unrelated Bash -> identical Read is hard-denied (regression for the aa4018c9 markdown deny, mirroring a live-session transcript)', () => {
    ...
    // Process 5: Read #2 -- identical offset/limit, same file, same session. Must be a hard deny (decision: block), never a silent pass-through of the full content again.
...
describe('sibling subagents get independent re-read dedup ledgers (regression: agent_id salting)', () => {
  it('a sibling subagent\'s genuinely-first read of a file already read by another subagent in the same session is not denied', () => {
    ...
    // A real Read always completes with a post_tool_use call, which is what stores the content snapshot the exact-overlap re-read deny revalidates against (defect B) -- without it there is no fingerprint to confirm agent-A's second read below is genuinely unchanged.
    expect(
    ...
    // Agent B (a sibling subagent -- SAME session_id, different agent_id) reads the SAME file for its own genuinely-first time. Before salting the persisted session-state key by agent_id, this was denied purely because Agent A had already read it: a false positive across sibling agents sharing one session_id.
    ...
    // Sanity: Agent A reading the SAME file a second time is still correctly hard-denied -- dedup must still work within one agent's own lineage.
    const preA2 = runHook('pre_tool_use', readPayload('agent-A'))
    expect(preA2.status).toBe(0)
    expect(preA2.stdout).toMatch(/"decision":"block"/)

    // Sanity: the main thread (no agent_id, same session_id) is its own independent lineage too -- its first read of the file is not denied by either subagent's reads.
...
describe('pre_compact manifest sees subagent edits (regression: agent_id salting drops them from the parent manifest)', () => {
  it('a file edited only by a sibling subagent still appears in the parent pre_compact manifest', () => {
    ...
    // A sibling subagent (same session_id, distinct agent_id) edits file B -- this lands in a SEPARATE agent-salted blob (`<sid>:agent:<aid>`), never the parent's.
    ...
    // pre_compact runs on the parent thread (no agent_id). Before the fix, its manifest only ever saw the plain unsalted blob and never knew file B was edited at all -- exactly the context compaction is supposed to preserve.
    const compact = runHook('pre_compact', { session_id: sessionId })
    expect(compact.status).toBe(0)
...
describe('"latest session" resolution skips subagent-salted blobs (regression: agent_id salting)', () => {
  it('session-summary with no explicit session id resolves to the parent session, not a newer subagent blob', () => {
    ...
    // A subagent (SAME session_id, distinct agent_id) is active AFTER the parent's last activity, so its blob is the newest file on disk by mtime.
    ...
    // Force deterministic mtime ordering (real-clock ordering across two spawned processes can tie on fast/low-resolution filesystems): bump the subagent's salted blob strictly into the future so this test reliably reproduces "subagent blob is the newest file on disk", the exact condition the bug depends on. The parent's own blob keeps its natural just-written mtime, which is already the most recent among every OTHER (non-agent-salted) session file from earlier tests in this shared tgHome.
    ...
    // Found by its salt marker rather than by reconstructing the filename: what this test is about is that session-summary resolves to the parent and not to a newer salted blob, and the agent id's exact spelling on disk is an implementation detail of sessionFileStem (it is a digest, so that the salt survives the 64-char cap). Reconstructing it here would pin this test to that spelling while testing nothing about it.
    const salted = fs.readdirSync(sessionsDir).filter((f) => f.startsWith(`${sessionId}_agent_`) && f.endsWith('.json'))
    ...
    // With no explicit session id, session-summary must still resolve to the genuine parent session, not the subagent's narrower, newer-on-disk blob.
...
describe('WebFetch recall survives the process boundary', () => {
  it('a pre_fetch of a previously fetched URL in a new process emits the web-output recall hint', () => {
    ...
    // Process 2 (cold memory): the recall hint must cross the process boundary via the persisted session state plus the on-disk web_outputs blob.
