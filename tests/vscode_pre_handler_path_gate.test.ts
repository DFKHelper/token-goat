 ...
 *
 * PROVENANCE: FORMAT-DERIVED. The canonical tool names are the values of VSCODE_TOOL_NAME_MAP in src/hooks_cli.ts, whose VS Code names (read_file, view_image, list_dir, grep_search, file_search, create_file, replace_string_in_file, insert_edit_into_file, edit_notebook_file) come from the languageModelTools entries in VS Code 1.136.0's resources/app/extensions/copilot/package.json, as cited on VSCODE_INPUT_KEY_MAP. VS Code sends no cwd and runs the hook in the workspace folder, which normalizePayload's vscode branch records as cwd; here the event carries the workspace as cwd directly. Paths and file contents are HAND-DERIVED.
 */
