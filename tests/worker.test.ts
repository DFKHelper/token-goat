...
afterEach(() => {
  // The retry-count DB helpers (bumpRetryCount/clearRetryCount in worker.ts) open a connection to `${DIR}/global.db` as a side effect of any transient-read-failure requeue or successful read, even for tests that inject a fake in-memory index callback and never otherwise touch the DB. Close it before removing DIR, or the still-open WAL handle makes rmSync fail with EPERM on Windows -- same pattern as the explicit closeDb(projectDb) calls elsewhere in this file, just applied unconditionally here since any test may now trigger it indirectly.
...
describe('isWorkerRunning', () => {
  ...
  // Regression-style coverage gap: pidAlive's EPERM branch (a pid that exists but is owned by another user, so process.kill(pid, 0) throws EPERM rather than succeeding or throwing ESRCH) was previously only asserted by inspecting the source's doc comment, not exercised by any test -- mutating that branch's `=== 'EPERM'` to a hardcoded `false` left the full worker test suite green. A process that exists but isn't ours must still read as alive (matching Python's os.kill semantics this ports), otherwise isWorkerRunning would misreport a real, running daemon as dead purely because of a permission boundary, and callers like ensureWorkerAlive/claimWorkerPidFile would wrongly spawn a duplicate.
  ...
  // Sibling to the EPERM case above: a genuinely dead pid raises ESRCH, which must read as not alive -- confirms the two error codes are not conflated by the same branch.
...
describe('ensureWorkerAlive (auto-heal regression)', () => {
  // Regression: before ensureWorkerAlive, startDetachedWorker was only ever invoked from the `worker start` CLI command -- nothing anywhere restarted a daemon that died (crash, a manual taskkill, a machine sleep/wake race, anything). A dead worker stayed dead indefinitely, with no automatic recovery, until a human happened to notice and ran `worker start` by hand.

  // The helper's failure path runs only when something has already gone wrong, so nothing would otherwise exercise it -- and a helper that threw while building its diagnostic would turn a legible flake into an inscrutable crash. Drive it directly against a pid that will never be alive.
  ...
  it('does nothing when a live worker running the current bundle is already running', () => {
    fs.writeFileSync(workerPidPath(DIR), `${process.pid}\n`)
    writeWorkerHeartbeat(DIR)
    // Stamp it as running the exact bundle ensureWorkerAlive would spawn right now -- otherwise the bundle-mismatch check below has nothing to compare against and (correctly) treats this as a pre-upgrade pid file, restarting it.
  ...
  // Regression: a running daemon was never compared against the bundle on disk, so an `npm install -g .` upgrade (or a rebuilt dist/token-goat.mjs) left the already-running worker executing the pre-upgrade code forever -- nothing else ever re-checks it, and the fix a user just installed silently never reaches the daemon until a reboot or a manual kill.
  ...
  // Regression: two installs sharing one data dir (e.g. the global npm install and a worktree or temp-copy dist/) each keyed the mismatch check on entry-script identity alone, so each saw the other's daemon as running a "different bundle" and restarted it -- at most once per 5-minute health marker, but still tearing down a perfectly live, correctly-versioned daemon that just happens to belong to a different install.
  ...
  // Regression: two hooks can judge the same stale daemon; if the other hook stops it and a fresh daemon claims the pid file before this one acts, re-reading the pid at stop time killed that fresh daemon and spawned a second one.
  it('leaves a daemon another hook just spawned alone when the pid file changes after the stale judgment', () => {
    const replacementPid = 999999998
    fs.writeFileSync(workerPidPath(DIR), `${process.pid}\n`)
    writeWorkerHeartbeat(DIR)
    const [entry, size, mtime] = currentDaemonStamp().split('|')
    fs.writeFileSync(workerStampPath(DIR), `${entry}|${size}|${Number(mtime) + 1}`)
    ...
    const killSpy = vi.spyOn(process, 'kill').mockImplementation(((pid: number, signal?: string | number) => {
      // The first liveness probe is the stale judgment; the other hook replaces the daemon right after it.
      if (!probed && pid === process.pid && signal === 0) {
        probed = true
        fs.writeFileSync(workerPidPath(DIR), `${replacementPid}\n`)
      }
      return true
    ...
    try {
      ...
      expect(killSpy).not.toHaveBeenCalledWith(replacementPid)
      expect(killSpy).not.toHaveBeenCalledWith(process.pid)
      expect(fs.readFileSync(workerPidPath(DIR), 'utf8').trim()).toBe(String(replacementPid))
    } finally {
      killSpy.mockRestore()
      stopWorker(DIR)
    }
  ...
  it('restarts a live worker with no stamp file at all (pre-upgrade pid-file format)', () => {
    fs.writeFileSync(workerPidPath(DIR), `${process.pid}\n`)
    writeWorkerHeartbeat(DIR)
    // No workerStampPath(DIR) write at all -- the exact shape of a pid file left by a daemon that predates this stamping mechanism.
    const killSpy = vi.spyOn(process, 'kill').mockImplementation(() => true)
    try {
      ensureWorkerAlive(DIR)
      expect(killSpy).toHaveBeenCalledWith(process.pid)
  ...
  it('spawns a fresh worker when the recorded pid is stale/dead', () => {
    ...
    ensureWorkerAlive(DIR)
    try {
      // Claiming the slot is synchronous in the parent, so it is the deterministic behavior this source-level test covers. The built-bundle daemon E2E test verifies child initialization and queue draining through the real CLI entrypoint.
      expect(fs.readFileSync(workerPidPath(DIR), 'utf8').trim()).not.toBe('999999999')
    } finally {
      // Stop the real spawned daemon before DIR is torn down in afterEach -- otherwise it keeps polling a directory that no longer exists.
      stopWorker(DIR)
    }
  })
  ...
  it('rate-limits repeated checks so a burst of calls does not re-spawn on every one', () => {
    fs.writeFileSync(workerPidPath(DIR), '999999999\n')
    // Simulate a healthcheck that already ran recently: a fresh marker file blocks the next check from even looking at the pid file, let alone attempting a restart.
  ...
  // Regression: TOKEN_GOAT_NO_WORKER_SPAWN is pinned to '1' by tests/setup/isolate-home.ts for the whole suite specifically so that any test exercising the postEditHandler hot path (e.g. hooks_edit.test.ts) never spawns a REAL detached daemon process as an incidental side effect of testing something unrelated -- but ensureWorkerAlive never actually read that env var, so it was a dead no-op: every such test silently spawned (and orphaned, until the daemon's own data-dir-deleted self-check eventually noticed and exited) a real child process. This file overrides the var back to unset above so its own deliberate real-spawn tests (this describe block, and the others above) are unaffected.
...
describe('stopWorker', () => {
  ...
  // Regression (double-daemon race): stopWorker used to delete the pid file unconditionally after killing the pid it read, with no re-check. Race: between stopWorker's kill and its rmSync, a concurrent `worker start` can observe the just-killed pid as dead, reclaim the pid-file slot via claimWorkerPidFile, and write a NEW daemon's pid into that same file -- which the still-running stopWorker call then deletes anyway, orphaning the new daemon (no pid file left for a later `worker stop` to find it by). A subsequent `worker start` would then "fix" the missing pid file by spawning a THIRD daemon, leaving two live daemons draining one queue. Simulate the race directly: after stopWorker's kill would have fired (this test's own live pid stands in for "the process stopWorker just killed"), have a concurrent claim overwrite the pid file with a different pid before stopWorker reaches its cleanup step -- proven here by writing the pid file back to a different value between the kill and the cleanup that the exit-handler-style guard must check.
  it('does not delete a pid file that a concurrent worker start already reclaimed for a new daemon', () => {
    fs.writeFileSync(workerPidPath(DIR), `${process.pid}\n`)
    writeWorkerHeartbeat(DIR)

    // Stand in for the race window: monkey-patch fs.rmSync so that, at the exact moment stopWorker is about to remove the pid file, we simulate a concurrent claimWorkerPidFile call having already reclaimed the slot for a brand-new daemon pid. If stopWorker's guard re-reads the pid file immediately before deleting (rather than deleting unconditionally), it must see the new pid and skip the delete entirely.
...
describe('claimWorkerPidFile (TOCTOU race regression)', () => {
  ...
  it('refuses and does NOT overwrite the pid file when a concurrent claim already recorded a live pid', () => {
    // Simulate the winning side of a race: a live process (this test process) claims the slot first, exactly as the first of two near-simultaneous `worker start` invocations would.
    expect(claimWorkerPidFile(DIR, process.pid)).toBe(true)
    writeWorkerHeartbeat(DIR)

    // A second, near-simultaneous `worker start` attempt tries to claim with a different pid. Before this fix, startDetachedWorker wrote the pid file with a plain fs.writeFileSync (no exclusive-create flag), so this second write would have silently clobbered the first daemon's pid file -- orphaning it with no pid file left for a later `worker stop` to find.
    expect(claimWorkerPidFile(DIR, 999999999)).toBe(false)

    // The file must still name the FIRST (live) pid, not the second writer's -- this is the exact clobbering behavior the fix closes.
    expect(fs.readFileSync(workerPidPath(DIR), 'utf8').trim()).toBe(String(process.pid))
  })
...
describe('processDirtyBatch', () => {
  ...
  // Regression: the returned count must reflect paths actually (re)indexed, not paths merely visited. An index callback that signals a no-op (returns `false`, mirroring the default indexer's sha-gate skip for byte-identical content) must not inflate the count.
  ...
  // Regression: worker.blocked_roots (set via `token-goat project exclude`) was validated from TOML and reported by `token-goat ignores`/`doctor`, but processDirtyBatch never consulted it -- a dirty-queued path under a blocked root was reindexed (or pruned) exactly like any other path.
  ...
  // Regression: for every dirty path in a batch, processDirtyBatch re-ran a full findProject ancestor-marker probe (9 markers x existsSync/lstat per level, plus isRepoContainer's own readdirSync) purely to feed lastKnownProjectRoots -- a value only consulted once per PRUNE_EVERY_N_DRAINS drains. Multiple dirty paths sharing the same directory in one batch repeated that walk once per path for an identical result. Assert findProject is called at most once per distinct dirname in the batch, not once per path.
  it('memoizes findProject per dirname within a batch instead of re-walking for every path', () => {
    ...
    // All three paths share the same dirname (DIR) -- pre-fix this was called 3 times (once per path); post-fix it must be called at most once for that one distinct dirname.
    expect(findProjectSpy.mock.calls.length).toBeLessThan(3)
    expect(findProjectSpy).toHaveBeenCalledTimes(1)
  })

  // Regression: processDirtyBatch is exported specifically for direct unit-test/caller use (see its doc comment), so it cannot assume every caller pre-filters blank entries the way parseDirtyQueueLines does for the real drain path. Without the `if (!p) continue` guard, an empty-string entry reaches fs.existsSync('') (false) and falls through to the remove callback with an empty path -- wasted work at best, and a surface a future remove() implementation could mishandle. Assert neither the index nor the remove callback is ever invoked for it.
  ...
  it('caps transient-read-failure retries for a permanently stuck path without starving healthy paths', () => {
    ...
    // The healthy path was indexed on the very first cycle and never reprocessed again -- it is not starved or repeatedly reprocessed just because the stuck path shares its batch.
    expect(indexed).toEqual([healthy])

    // The stuck path eventually stops being requeued -- the queue empties out instead of holding it forever.
    expect(getDirtyPathsFor(DIR)).toEqual([])
  ...
  it('gives a fresh retry budget to a path re-dirtied after exhausting its retry cap', () => {
    ...
    // Simulate the edit-driven re-dirty path: reset the retry count (what appendDirtyPath now does) and re-queue the still-stuck path, as a fresh edit to it would.
  ...
  it('persists the retry-count reset through the DB even when every in-process DB connection is closed and reopened between steps (cross-process simulation)', () => {
    ...
    // Close every cached DB connection -- simulates the daemon process (which just wrote retry_count = 5 while giving up) exiting or being a wholly separate process from whatever resets the count next.
    clearModuleCaches()

    // Simulates appendDirtyPath running in a fresh, short-lived hook CLI process: no shared memory with whatever wrote the exhausted count, only DB access.
    clearRetryCount(dbPath, lockedPath)
    writeQueue(DIR, [lockedPath])

    // Close the connection the reset just opened too, then simulate the daemon coming back (its own fresh process/connection) to drain the re-queued path.
    clearModuleCaches()
...
describe('drainOnce', () => {
  ...
  // Regression: doctor's dirty-queue drain-heartbeat check (checkDirtyQueueHealth) distinguishes a running-but-wedged worker from one actively draining by the recency of this marker's mtime. It must be touched on EVERY cycle, including a no-op one (empty queue) -- a wedged loop that stopped reaching the end of drainOnce is exactly what the check needs to catch, and an empty-queue cycle is the most common steady-state case in a real project.
  ...
  it('updates the drain-heartbeat marker mtime on each subsequent cycle', () => {
    ...
    // Back-date the marker so a second real drainOnce call is guaranteed to produce a strictly later mtime even on filesystems with coarse mtime resolution.
  ...
  // End-to-end proof for the Terraform/HCL adapter (feature-queue #310): drives the real shipping path (drainOnce with no injected index callback -> indexFileSync -> extractTerraform) rather than calling extractTerraform directly, per CLAUDE.md's injected-seam-trap warning -- a unit test on the extractor alone would never catch a registration gap (missing Language union entry, missing EXTENSION_LANGUAGE mapping, or missing NO_TREE_SITTER_EXTRACTORS wiring) that leaves .tf files unreachable via the real drain -> index -> symbols table path even though the extractor itself works in isolation.
  ...
  // Same shipping-path proof as the .tf test above, for the bash adapter (feature-queue #332): .sh/.bash already mapped to the 'bash' Language via EXTENSION_LANGUAGE, but had no entry in NO_TREE_SITTER_EXTRACTORS, so every shell script indexed to zero symbols via the real drain -> index -> symbols table path even though extractBash worked correctly in isolation.
  ...
  // Regression: indexFileSync/cmdIndex's skip_dirs and large_file_skip_kb early-returns left stale symbol/ref rows behind AND never cleared files.sha, so a file indexed while small/ allowed that later crosses the cap (or whose directory is newly added to skip_dirs) got stuck in a self-perpetuating loop: the stale sha never matched current content, so every subsequent drainOnce re-selected it as changed, re-called indexFileSync, which re-skipped it again, forever, while the OLD rows kept resolving via symbol/read. This drives the real default drain path end-to-end (no injected index callback): index a file normally, EDIT it (so its sha changes and drainOnce's parseUnchanged gate does not skip the re-check) while also lowering large_file_skip_kb below its new size, re-drain, and assert (a) the stale symbol no longer resolves and (b) the files row is gone (sha cleared) so the file settles into a stable not-indexed state instead of endlessly re-processing.
  it('default path clears stale rows and files.sha when a previously-indexed file becomes skip-eligible (no injected callback)', () => {
    ...
    try {
      ...
      // Edit the file (sha changes, so the parseUnchanged gate in makeIndexer will not skip the re-check) and simultaneously make it skip-eligible by lowering large_file_skip_kb below its new size -- simulating the file crossing the cap (or its directory being newly added to skip_dirs) on a real edit.
      fs.writeFileSync(
        src,
      ...
      // The stale symbol must no longer resolve, and the files row (including sha) must be cleared -- not just the symbols/refs rows -- so the file settles into a stable not-indexed state instead of being endlessly re-selected as changed.
      ...
      // A further re-queue+drain while still skip-eligible must settle at the same stable state -- no stale rows resurrect and no error is thrown -- rather than looping forever.
      writeQueue(DIR, [norm])
      expect(drainOnce(DIR)).toBe(1)
      expect(querySymbols({ name: 'growableWorkerSymbol', limit: 10 }, projectDb).length).toBe(0)
      expect(getFileEntry(norm, projectDb)).toBeNull()
  ...
  // Regression: makeIndexer's parseUnchanged sha-gate ran indexFileSync ONLY when content changed, so a file re-enqueued with BYTE-IDENTICAL content (same sha) after becoming skip-eligible purely from a config change (skip_dirs/large_file_skip_kb, no edit at all) never had indexFileSync called -- and the stale-row purge lives INSIDE indexFileSync -- so its symbols/refs/files rows were never cleared, even though embeddings correctly purge via their own independent wouldSkipFileEmbedding gate. This is the specific gap the sibling "becomes skip-eligible" test above does NOT cover, since that test also edits the file (changing its sha) to make parseUnchanged false, bypassing this exact code path. Drives the real default drain path end-to-end (no injected index callback, no content change).
  it('default path clears stale rows for a skip-eligible file re-enqueued with unchanged content (parseUnchanged=true)', () => {
    ...
    try {
      ...
      // Make the file skip-eligible via a CONFIG CHANGE ONLY -- content and sha are untouched, so makeIndexer's parseUnchanged gate would evaluate true (entry.sha === sha), exactly the gap this test targets.
      vi.mocked(loadConfig).mockReturnValue({
        worker: { blocked_roots: [] },
        indexing: { skip_dirs: [], skip_files: [], large_file_skip_kb: 0, large_file_symbol_only_kb: 1048576 },
      } as unknown as ReturnType<typeof loadConfig>)
      ...
      // The stale symbol must no longer resolve, and the files row (including sha) must be cleared -- consistent with the embeddings side, which already purges correctly regardless of parseUnchanged via its own independent gate.
      expect(
        querySymbols({ name: 'unchangedWorkerSymbol', limit: 10 }, projectDb).length,
      ).toBe(0)
      expect(getFileEntry(norm, projectDb)).toBeNull()
  ...
  // Regression: a rename/delete that never goes through the Edit hook path (git mv, a directory rename, `git checkout <branch>`, `git clean`) never enqueues the old path, so the dirty-queue reconciliation exercised by the sibling "prunes a deleted file's rows on re-drain" test above never runs for it -- the row orphans in the index forever. drainOnce now also runs an opportunistic pruneDeletedFiles sweep every PRUNE_EVERY_N_DRAINS (30) cycles, scoped to the project root it learned from earlier dirty-queue traffic. This drives the real default path end-to-end: index a file (learning the project root along the way), delete it WITHOUT re-queuing its path, drive enough drain cycles to cross the periodic threshold, and assert the stale row is gone. Fails pre-fix (row survives indefinitely, since nothing else in the drain loop ever revisits it) and passes post-fix.
  it('prunes a stale row for a file renamed/deleted outside the Edit hook path once enough drain cycles cross the periodic threshold', () => {
    // A project marker so findProject(path.dirname(src)) resolves DIR as a real project root -- opportunistically learned by processDirtyBatch from the dirty paths it processes.
    ...
    try {
      ...
      // Simulate the git-mv/git-clean scenario: the file is gone from disk, but its old path is never re-enqueued (unlike the sibling test above, which explicitly re-queues the old path to exercise the ordinary dirty-queue reconciliation branch instead of this one).
      fs.rmSync(src)

      // Empty-queue drain cycles: nothing for the normal dirty-queue path to reconcile each time, so only the periodic prune sweep (using the project root learned by the very first drainOnce call above) can catch the stale row.
      for (let i = 0; i < 30; i++) {
        drainOnce(DIR)
      }
  ...
  it('default path skips re-indexing a file whose content is unchanged (sha gate)', () => {
    ...
    try {
      ...
      // This test targets the PARSE-sha gate specifically (embedding freshness is gated independently -- see the dedicated embed_sha regression tests below). Stamp embed_sha to match the stored sha, simulating a prior successful embed, so a fully-skipped drain here isolates the parse gate rather than being driven by the (unrelated) embed re-trigger.
      getDb(projectDb).prepare('UPDATE files SET embed_sha = sha WHERE path = ?').run(norm)

      // Re-queue the unchanged file and drain again; the sha gate must skip the reparse, and the returned count must not include it (it was visited but not actually reindexed).
      writeQueue(DIR, [norm])
      expect(drainOnce(DIR)).toBe(0)
      expect(querySymbols({ name: 'shaGatedSymbol', limit: 10 }, projectDb).length).toBe(0)
    } finally {
  ...
  // Regression: indexFileSync stored the SHA of the utf8-DECODED content string (invalid byte sequences replaced with U+FFFD, then re-encoded), while processDirtyBatch's gate SHA (fingerprintFile) hashes the RAW file bytes directly. For a file containing an invalid-UTF-8 byte, those two hashes could never match, so the sha gate above never engaged and the file was fully reparsed on every single drain even when byte-for-byte unchanged.
  it('sha gate still skips an unchanged file whose raw bytes are not valid UTF-8', () => {
    ...
    try {
      ...
      // Isolate the parse gate from the (independently gated) embed re-trigger -- see the matching comment in the sha-gate test above.
      getDb(projectDb).prepare('UPDATE files SET embed_sha = sha WHERE path = ?').run(norm)
  ...
  // Regression: parseFile stripped a leading UTF-8 BOM (U+FEFF) before parsing, but indexFileSync (the worker's real shipping path -- used by drainOnce) called parseContent directly on the raw decoded content without stripping it first. Any `^`-anchored regex extractor (markdown headings included) then has its line-1 match fail, because the BOM sits before the `#`. This drives the real default drain path (no injected callback) on a BOM'd markdown file and asserts the line-1 heading still resolves as a symbol. It fails pre-fix (0 symbols) and passes once the BOM is stripped inside parseContent itself, so both parseFile and indexFileSync inherit the fix.
  ...
  describe('drainOnce atomic rename-to-claim (lost-update regression)', () => {
    ...
    // Regression (mutation-testing gap): the post-claim re-read forwards only the *new suffix* of the claimed file (recheck.slice(claimedContent.length)), not the whole re-read content, to the live queue. This matters specifically for a write landing on the SAME renamed claim-target file mid-batch (the FILE_SHARE_DELETE/rename-race window the doc comment above describes) rather than a fresh dirty.txt recreated after the claim (the prior test). Without the startsWith-slice, an already-processed path in the claimed content would be re-forwarded into the live queue alongside the genuinely-new one, redundantly reprocessing it on the next cycle instead of forwarding only what the race actually added.
    it('forwards only the new suffix of the claimed file to the live queue, not the whole re-read content, when a write lands on the claim target itself mid-batch', () => {
      ...
      // Only G (the genuinely new suffix) is forwarded to the live queue -- A must not be redundantly re-added just because the whole claimed file was re-read.
    ...
    it('bumps the transient-retry count only once per drainOnce call when stage (a) recovering an abandoned .draining file requeues a path stage (b) of the SAME cycle would otherwise immediately reclaim', () => {
      ...
      // The path is still requeued for the next drain cycle either way -- the fix defers WHEN the append lands in the live queue, not WHETHER it does.
      expect(getDirtyPathsFor(DIR)).toEqual([lockedPath])
    })

    it('recovers multiple abandoned .draining/.draining.alt-<ts> files in a single cycle', () => {
      // Regression: before listDrainingFiles, drainOnce only ever checked the single primary `.draining` name. A `.draining.alt-<ts>` fallback file (left behind when stage (b) had to claim under a fallback name because the primary was still occupied by a file a previous cycle could not clean up) would then sit on disk forever, unprocessed, on every future cycle -- listDrainingFiles must recover ALL of them, not just the first.
    ...
    // Regression (mutation-testing gap): listDrainingFiles must exclude a `.draining.alt-<ts>` file that was itself already quarantined (renamed to `...alt-<ts>.corrupt-<ts2>` by drainOnce's own cleanup-failure fallback). Without the exclusion, a quarantined file's name still matches the `.alt-` prefix check, so it would be picked back up and reprocessed as live draining content on the very next cycle instead of being left alone for cleanupWorkerStateFiles' age-based sweep to eventually remove it.
    it('does not reprocess an already-quarantined `.draining.alt-<ts>.corrupt-<ts2>` file', () => {
      ...
      fs.mkdirSync(path.dirname(drainingPath), { recursive: true })
      fs.writeFileSync(drainingPath, `${D}\n`)
      // Simulate an already-quarantined alt file left behind by a prior cycle -- its content must never be treated as a real path to index.
      ...
      expect(fs.existsSync(drainingPath)).toBe(false)
      // The quarantined file is left untouched by drainOnce -- only cleanupWorkerStateFiles' age-based sweep may eventually remove it.
  ...
  describe('drainOnce rm-after-process (crash-safety regression)', () => {
    it('does not lose claimed queue paths when processing crashes mid-batch (stage b)', () => {
      // Regression: drainOnce used to delete the claimed .draining file BEFORE running processDirtyBatch, so a crash (simulated here as the index callback throwing) partway through a batch already had the queue file deleted -- the paths were lost forever until some unrelated future edit re-queued them. The fix defers the rm until after processDirtyBatch completes successfully.
      const A = path.join(DIR, 'a.ts')
      const B = path.join(DIR, 'b.ts')
      fs.writeFileSync(A, 'export const a = 1\n')
      fs.writeFileSync(B, 'export const b = 2\n')
      ...
      // Pre-fix, the claimed .draining file was already removed before processDirtyBatch ran, so it would be gone here even though the batch never finished.
    ...
    it('does not lose recovered .draining paths when processing crashes mid-batch (stage a)', () => {
      // Same regression as above, exercised via the crash-recovery path: an abandoned .draining file (from a previous crashed drain) must also survive a crash during ITS OWN processDirtyBatch run, rather than being deleted up front.
      const C = path.join(DIR, 'c.ts')
...
// Regression: makeIndexer's catch block used to swallow a genuine indexFileSync failure with zero logging anywhere, and the caught exception's implicit `undefined` return was `!== false`, so processDirtyBatch counted the failed file as successfully indexed and dequeued it from the dirty queue forever -- no error surfaced anywhere, and the file's index silently went stale for good. This drives the real shipping path: `drainOnce(DIR)` with NO injected index/remove callbacks, so the actual default `makeIndexer` -> `indexFileSync` pipeline runs. Only `indexFileSync` itself is mocked (genuinely throwing for one specific path, genuinely calling through to the real implementation for the other), which is the narrowest possible seam for deterministic failure injection -- the orchestration under test (makeIndexer, processDirtyBatch, drainOnce) is entirely real.
describe('makeIndexer failure handling (regression)', () => {
  ...
  it('logs a swallowed indexFileSync failure, does not count it as indexed, and still indexes the rest of the batch', () => {
    ...
    try {
      ...
      // (a) The swallowed failure must be surfaced somewhere discoverable: the worker's error log, since the detached worker process's stdio is discarded (startDetachedWorker uses `stdio: 'ignore'`) and nothing in this file otherwise logs anything.
      const logPath = path.join(DIR, 'worker-errors.log')
...
// Regression: makeIndexer committed files.sha via indexFileSync, then fired indexFileEmbeddings without awaiting it (fire-and-forget). If the daemon died mid-embedding, or the embed call threw (previously swallowed with nothing to show for it), the sha was already stamped current, so the next touch of IDENTICAL content got sha-gate-skipped forever -- chunks stayed stale/missing permanently, with no way to recover short of deleting the index. files.embed_sha now tracks embedding freshness separately from files.sha (parse freshness): it is only stamped after indexFileEmbeddings actually commits successfully (see its doc comment in parser.ts), and makeIndexer's gate re-triggers embedding whenever embed_sha !== sha, even when the parse-sha gate above it skipped the reparse entirely. This drives the real default drain path (drainOnce(DIR), no injected index callback); only indexFileEmbeddings itself is mocked (throwing once to simulate the crash/error, then delegating to the real implementation), the narrowest seam for deterministic failure injection -- makeIndexer/processDirtyBatch/drainOnce and indexFileSync are entirely real.
describe('makeIndexer embed-freshness gate (regression)', () => {
  ...
  it('retries embedding on the next touch of unchanged content after a simulated embedding crash', async () => {
    ...
    try {
      // First drain: indexFileSync succeeds (real), but the embedding step throws. The syntactic index must still succeed -- an embeddings-only failure never fails the overall index -- and embed_sha must be left unstamped (stale/empty) rather than wrongly marked current.
      writeQueue(DIR, [norm])
      expect(drainOnce(DIR)).toBe(1)
      ...
      // Let the first (rejected) embed call's promise chain settle and clear itself from worker.ts's per-file in-flight map (embedFileSerialized) before the second drain -- otherwise this second drain would be legitimately treated as overlapping the still in-flight first call and chained behind it rather than dispatched immediately.
      await Promise.resolve()
      await Promise.resolve()
      await Promise.resolve()

      // Second drain of the SAME, byte-identical file: the parse-sha gate alone would skip it entirely (pre-fix behaviour), permanently losing the embedding retry. With the fix, embed_sha (still empty) !== sha, so embedding must be retried even though the parse step is correctly skipped.
  ...
  // Regression (#5): indexFileEmbeddings (parser.ts) returned before ever reaching the embed_sha UPDATE when indexing.embeddings_enabled is false, so embed_sha was left stale/empty forever. That meant embedUnchanged (this gate, above) could never hold for these files: every re-touch of byte-identical content re-entered indexFileEmbeddings just to hit the same early-return again, on every drain, for as long as embeddings stayed disabled -- harmless correctness-wise, but wasted work.
  //
  // The fix stamps embed_sha with a disabled-marker form of the sha (disabledEmbedSha(sha), NOT the bare sha) on the disabled early-return, and this gate now compares against that same marker form while disabled. Stamping the bare sha directly (the naive fix) would have been indistinguishable from a real successful embed: re-enabling embeddings later for the same unchanged content would then wrongly look "already embedded" and permanently skip its real first embed -- the second test below proves that scenario specifically. Drives the real default drain path (drainOnce(DIR), no injected index callback); only indexFileEmbeddings is spied (call-through, not mocked) to count invocations without altering its real behaviour.
  it('stamps a disabled-marker embed_sha while embeddings are disabled, so a second reindex of unchanged content short-circuits via the gate', () => {
    ...
    try {
      // First drain: symbols index normally; indexFileEmbeddings is invoked once but no-ops (embeddings disabled) other than stamping the disabled-marker embed_sha.
      writeQueue(DIR, [norm])
      expect(drainOnce(DIR)).toBe(1)
      expect(embedSpy).toHaveBeenCalledTimes(1)
      expect(
      ...
      // Second drain of the SAME, byte-identical file, still disabled: parseUnchanged AND embedUnchanged both now hold, so makeIndexer must short-circuit entirely without invoking indexFileEmbeddings again.
      writeQueue(DIR, [norm])
      drainOnce(DIR)
      expect(embedSpy).toHaveBeenCalledTimes(1)
    } finally {
  ...
  it('re-embeds unchanged content on the next touch after embeddings are re-enabled, instead of being masked by a disabled-marker embed_sha from an earlier disabled run (fail-on-buggy: stamping the bare sha while disabled would make this look "already embedded" forever)', async () => {
    ...
    try {
      ...
      // Let the first embed call's promise chain settle and clear itself from worker.ts's per-file in-flight map (embedFileSerialized) before the second drain -- otherwise this second drain would be legitimately treated as overlapping the still in-flight first call and chained behind it rather than dispatched immediately.
      await Promise.resolve()
      await Promise.resolve()
      await Promise.resolve()

      // Second drain of the SAME, byte-identical file, now with embeddings enabled: the gate must NOT treat the disabled-marker embed_sha as a match for a real sha comparison -- indexFileEmbeddings must be invoked again so the file actually gets embedded.
      vi.mocked(loadConfig).mockReturnValue({
        worker: { blocked_roots: [] },
  ...
  // Regression: two drains of a rapidly re-edited file could spawn two concurrent indexFileEmbeddings promises for the same path (makeIndexer fires embedding off without awaiting it, by design -- see its doc comment). If the OLDER call (started first, with now-stale content) happened to finish AFTER the newer one, its stale chunks/vectors silently overwrote the fresher ones -- last-writer-wins, self-correcting only on the NEXT edit. The fix chains same-file embed calls onto one another so they always settle in submission order. Drives the real default drain path (drainOnce(DIR), no injected index callback -- makeIndexer's actual production wiring); only indexFileEmbeddings itself is mocked, with each call's completion under this test's explicit control, so the assertions prove the ORDER the production code dispatches and resolves the two calls in, not any artifact of the mock's own timing.
  it('serializes concurrent embedding calls for the same file so a slower older call cannot overwrite a faster newer one', async () => {
    ...
    // First drain: v1 content (sha1). Its embed call is dispatched but deliberately left pending -- nothing resolves it yet. The dispatch itself is chained through a `.then()` (a microtask, even off an already-resolved "no prior call" base promise), so flush one tick before checking invocations.
    writeQueue(DIR, [norm])
    expect(drainOnce(DIR)).toBe(1)
    await Promise.resolve()
    ...
    // Second drain: rewrite to v2 content (a different sha) while the first embed call for sha1 is still in flight -- simulates a rapid re-edit racing an in-flight embed of the now-stale content.
    ...
    // With serialization, the second call must not even be dispatched to indexFileEmbeddings yet -- it is chained behind the still-pending first call, not racing it concurrently.
    expect(invocations).toEqual([sha1])

    // Resolving the first (older) call is what unblocks the second -- proving a chain, not an independent race, governs dispatch order.
    ...
    // Final commit order must be submission order: sha1 (older, submitted and resolved first) then sha2 (newer, resolved last) -- never the reverse, which would mean the fresher content lost to stale content.
    expect(commits).toEqual([sha1, sha2])
  })

  // Regression (mutation-testing gap): embedFileSerialized's cleanup only deletes the inFlightEmbeddings map entry for a settling call when that call is STILL the current entry -- an older call's cleanup running after a newer call has already replaced the map entry must be a no-op, not an unconditional delete. Exercises a THREE-generation chain (the two-generation test above cannot distinguish this: with only two calls, the older one's cleanup fires after the test has already finished asserting). Here, call1 resolves while call2 is still pending, which is exactly the point where call1's cleanup handler runs against a map entry that already names call2 -- and a THIRD edit arriving right after must still see call2 as `prior` and chain behind it, not race it, to prove the map entry for call2 survived call1's cleanup.
  it('does not let an older call\'s cleanup delete a newer call\'s still-in-flight map entry out from under a third, later call', async () => {
    ...
    // Resolve v1 -- this is what dispatches v2 AND fires v1's own cleanup handler. Flush several ticks so both settle before the third edit arrives.
    pendingResolvers.get(sha1)!()
    await Promise.resolve()
    await Promise.resolve()
    await Promise.resolve()
    ...
    // v3: v2 is still pending (unresolved). If v1's cleanup wrongly deleted the map entry for v2, this third call would see no `prior` and dispatch immediately, racing v2 instead of chaining behind it.
    ...
    // v3 must NOT have been dispatched yet -- it is chained behind the still-pending v2, not racing it concurrently.
  ...
  // Regression (#250): inFlightEmbeddings above only serializes duplicate work on the SAME file -- a dirty batch of N distinct changed files previously fired N concurrent indexFileEmbeddings pipelines with no global cap at all. The fix routes every embed call through a global slot counter honoring config.worker.max_pool_workers, queuing callers beyond the cap instead of firing them all at once. Drives the real default drain path (drainOnce(DIR), no injected index callback -- makeIndexer's actual production wiring); only indexFileEmbeddings is mocked, with each call's completion under this test's explicit control, so the assertions prove the production code never dispatches more than the configured cap concurrently, not an artifact of the mock's own timing.
  it('caps concurrent embedding pipelines across different files at worker.max_pool_workers, queuing the rest', async () => {
    // Reset worker.ts's process-wide embed-slot counter: an earlier test in this file may have dispatched a real (unresolved-by-test-end) embed call without awaiting it to settle, which would otherwise leave activeEmbedSlots polluted going into this test.
    clearModuleCaches()
    vi.mocked(loadConfig).mockReturnValue({
    ...
    // makeIndexer fires embedding fire-and-forget with no `await` in the drain loop, and embedFileSerialized dispatches synchronously (no microtask hop) whenever a slot is immediately free -- so with a cap of 2, only the first two of these three distinct files are dispatched to indexFileEmbeddings inline within drainOnce itself. The third is queued behind the cap, not fired concurrently, with no await needed to observe this.
  ...
  // Regression: the release closure attached to an already-dispatched embed survived the reset that zeroed the counter it decrements. When that stale call settled it took the fresh counter negative, and a negative count means `activeEmbedSlots < limit` stays true for one extra caller per stale task -- so the global cap this gate exists to enforce was quietly loosened for the rest of the process. Drives the real drain path with only indexFileEmbeddings spied.
...
`)
    ...
    // Two fresh files, cap of 1: exactly one may dispatch. A counter left at -1 by the stale release would admit both.
  ...
  // Regression: the reset registered beside the slot counter cleared the counter and the waiter queue but not inFlightEmbeddings. A call queued behind the cap returns a promise whose only way to settle is the closure sitting in that queue, so discarding the queue orphaned the promise: its cleanup never ran, its map entry never cleared, and pendingEmbeddings() -- which waits on exactly those values -- never resolved. A later test awaiting it hung until the suite timeout, which reads as a flake rather than as this test leaving state behind. Drives the real drain path, same as the cap test above; only indexFileEmbeddings is mocked.
...
`)
    ...
    writeQueue(DIR, files)
    expect(drainOnce(DIR)).toBe(2)
    // With a cap of 1 the second file is queued, never dispatched -- exactly the state whose tracking the reset has to drop along with the queue it depends on.
    expect(invoked).toEqual([files[0]])
...
// Regression (mutation-testing gap): the loop body re-checks shouldStop() immediately after the drain/housekeeping work, BEFORE the setTimeout poll-interval sleep, so a stop request that arrives mid-cycle exits right away instead of waiting out one more full pollIntervalMs sleep for nothing. Without that second check, the loop always sleeps once more before the next `while (!shouldStop())` iteration catches it -- a real, measurable shutdown-latency regression for a caller (e.g. stopWorker's own SIGTERM-driven exit path) that wants the daemon to stop promptly. Uses a deliberately large pollIntervalMs and asserts on wall-clock elapsed time so a wasted sleep is unambiguous rather than lost in test-runner jitter.
describe('runWorkerLoop prompt-stop (mutation-testing gap)', () => {
  it('stops immediately after the loop body when told to, without waiting out one more poll interval', async () => {
    ...
    let resolved = false
    // Await the loop's own promise chain (drainOnce + housekeeping are synchronous/microtask work) without ever advancing real or fake time: if the second shouldStop() check is skipped, the loop falls through to `setTimeout(resolve, pollIntervalMs)` and this would never resolve without a 2000ms timer advance -- proving the behavior directly instead of inferring it from a wall-clock bound.
...
// Regression: runWorkerLoop never checked whether its own data dir still existed, so a detached daemon spawned against an ephemeral/scratch directory (e.g. `index --walk` in a temp dir during dogfooding, or a test that never called `worker stop`) ran forever once that directory was deleted out from under it -- confirmed in practice as 524 stray `--worker-daemon` processes accumulated over two weeks. `shouldStop` here always returns false (mirroring a real daemon, which has no external stop signal until `worker stop` sends SIGTERM), so the only thing that can end this loop is the new self-existence check.
describe('runWorkerLoop self-terminates when its data dir is deleted (regression)', () => {
  it('exits on its own once the data dir no longer exists, even though shouldStop never fires', async () => {
    ...
    try {
      ...
      // Both `lastSnapshotCleanupMs`/`lastKnownRootsSweepMs` start at 0, so the very first loop iteration always fires the known-roots sweep too, which opens (and caches) a SQLite connection to `${scratchDir}/global.db` -- release it first, same as this file's own afterEach does for DIR, or the still-open native handle makes the directory removal below fail with EPERM on Windows.
      closeDb(path.join(scratchDir, 'global.db'))
      // maxRetries/retryDelay: Windows can transiently EPERM a recursive rmSync on a directory whose contents a background process (this test's own runWorkerLoop, still mid-cycle) just touched -- retry rather than flake.
      ...
      // Best-effort: the test body above already removed scratchDir; a second removal attempt can spuriously EPERM on Windows (transient AV/handle lock on an already-deleted temp dir) and must not mask the assertion result above.
      try {
        fs.rmSync(scratchDir, { recursive: true, force: true, maxRetries: 5, retryDelay: 50 })
      } catch {
        // ignore
...
describe('runWorkerLoop self-terminates when another daemon takes over its pid file (regression)', () => {
  it('exits once the pid file it owned names a different pid, with the dir still present', async () => {
    ...
    try {
      ...
      const result = await Promise.race([loopPromise.then(() => 'stopped'), timeout])
      // Release the global.db handle the first-iteration known-roots sweep opened, so the temp dir removal in finally does not EPERM on Windows -- same reason as the data-dir test above.
      closeDb(path.join(scratchDir, 'global.db'))
      expect(result).toBe('stopped')
    } finally {
      closeDb(path.join(scratchDir, 'global.db'))
  ...
  // Guard the takeover check against a transient read: readPidFile collapses a missing, empty, or mid-write pid file to null (pid-file replacement is not atomic to a concurrent reader). The loop must NOT read that null as lost ownership, or a single filesystem hiccup would terminate the sole legitimate daemon -- the exact leak this check exists to prevent, self-inflicted. Only a concrete different pid ends the loop; an emptied pid file leaves an owner running until shouldStop or a real successor writes its own pid.
  it('keeps running when the pid file it owned is emptied (transient null is not a takeover)', async () => {
    ...
    try {
      ...
      // Race the loop against a window several poll intervals wide. The loop must still be running: the timer wins ('running'). If it exits here ('stopped'), the null was wrongly treated as a takeover -- which is the pre-fix behavior of `else if (ownedPidFile) break`.
...
// Regression: cleanup_session/cleanup_stale existed in snapshots.ts but nothing ever called them -- session_snapshots/<sessionId>/ directories accumulated forever. runWorkerLoop now sweeps stale session snapshots on the same periodic loop as the dirty-queue drain. This drives the real default path (no injected cleanup callback, the shipping path) end-to-end: write a snapshot, backdate its mtime past cleanup_stale's 24h staleness window, run one real loop cycle, and assert the stale snapshot was actually removed from disk.
...
// Regression: worker-errors.log had no rotation mechanism -- it could grow unbounded over a project's index lifetime. runWorkerLoop now rotates it via cleanupWorkerStateFiles on the same periodic sweep as cleanup_stale above. This drives the real default path end-to-end: write an oversized worker-errors.log, run one real loop cycle, and assert it was actually rotated.
...
// Regression (mutation-testing gap): cleanupWorkerStateFiles' `.corrupt-*` quarantine cleanup half had zero test coverage, even end-to-end -- only the worker-errors.log rotation half was exercised above. A mutation removing the `.corrupt-` filter entirely (so every file in the queue dir, corrupt-marked or not, becomes eligible for age-based deletion) still passed the full suite. That's a real production risk: cleanupWorkerStateFiles runs periodically inside runWorkerLoop, in the same directory as the live dirty queue and drain-heartbeat marker.
...
// Regression: before this, pruneDeletedFiles only ever ran via the manual `token-goat index` CLI command -- nothing periodic pruned the shared global.db, so it could accumulate dead file rows indefinitely. runWorkerLoop now runs sweepKnownRoots on the same periodic-sweep mechanism as the snapshot cleanup above. Drives the real default path end-to-end: index a file, record its project root as known, delete the file, run one real loop cycle, and assert its rows were actually pruned.
...
describe('resolvePollIntervalMs', () => {
  ...
  // The regression: startDetachedWorker skipped the env entirely and wrote the default straight into the child's environment, so TG_WORKER_POLL_MS was inert on the one path that starts a real daemon, even though the daemon itself reads that exact variable.
...
// Regression: the id-keyed disk caches had no automatic reaper. storeBlob prunes the subdir it just wrote, but session state is written outside that funnel (session_store.ts writes through sessionPath, not storeBlob), so ~/.token-goat/sessions grew without any bound -- 83k files on the author's machine, which every hook that lists sibling session states then had to readdir past. runWorkerLoop now calls sweepCacheRoots on the same periodic sweep as the snapshot cleanup. This drives the real default path end-to-end (no injected sweep callback): write a stale session blob, run one real loop cycle, and assert it was actually removed from disk.
describe('runWorkerLoop cache sweep (regression)', () => {
  it('ages out a stale session blob via the default periodic loop', async () => {
    ...
    // A live session's blob is rewritten on every hook, so a fresh mtime means the conversation is still going: the sweep must never take it.
