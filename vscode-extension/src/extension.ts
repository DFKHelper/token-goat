...
const CANNED_PROMPTS: Record<string, string> = {
  ...
  explain: `First decide what this text is: an error message or log excerpt, or something else (a changelog, a note, documentation). If it is an error, structure the answer as: "What happened" (one or two sentences), "What to do" (numbered steps, easiest fix first), "If that doesn't work" (one line on the next step). If it is NOT an error, do not invent a problem or steps — explain in plain language what the text says and end with one line answering "Does this need me to do anything?" (usually: no). Never describe a failure the text does not report. ${PLAIN_STYLE}`,
