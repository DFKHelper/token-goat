...
| BE-12 | Whether OpenClaw's `after_tool_call` and pi's `tool_result` can mutate the tool result at all -- both bridges fire post_tool_use for side effects and ignore the response, so every post rewrite (fencing, redaction, compression) is dropped there today; opencode's same-shaped gap was fixable because its API demonstrably supports output mutation, these two are unproven | the plugin APIs' own source (result-mutation surface), or a live instance |
