...
| SC-22 | 51 | The hook output-channel partition, producer-side: every non-pass HookOutput shape enumerated from the real producing handlers (deny pre-only; context on 6 events with 3 serializations; rewriteInput pre-only; rewriteOutput post-only; subagent_stop/post_compact pass-only; notification/stop producerless) and mapped per harness in docs/hook-channel-matrix.md with per-cell evidence classes. Two defects found and fixed (loop-49 spawn advisory emitted into Copilot's dropped post additionalContext channel with wrong-schema content; opencode plugin dropped every rewriteOutput), one dead mechanism recorded for a next loop (image shrink on Copilot/OpenClaw, reroutable via the opencode/pi materialize pattern), harness-design drops recorded (grok deny-only, kimi no-rewrite/post-ignored, openclaw/pi post-ignored), unverified cells filed as BE-06/BE-10..BE-13 rather than papered over | a bridge adds or changes an event/response translation, or a new HookOutput shape lands |
## Deliberately Left
