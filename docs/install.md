...
This registers token-goat's MCP server in VS Code's user `mcp.json` (a timestamped `.bak` is written before any change to an existing file) and installs agent hooks in `~/.copilot/hooks/` (`token-goat.json` plus the `token-goat-shim.js` it runs). VS Code's Copilot agent reads hooks from that folder, so token-goat sees the agent's built-in tool calls: `read_file`, `view_image`, `list_dir`, `grep_search`, `file_search`, `create_file`, `replace_string_in_file`, `insert_edit_into_file`, `edit_notebook_file`, and `run_in_terminal`. The routing guidance for the agent goes to `~/.copilot/instructions/token-goat.instructions.md`, a personal instructions file VS Code applies in every workspace. Add `-p`/`--project` to install for the current project instead: the MCP entry goes to `.vscode/mcp.json`, the hooks to `.github/hooks/`, and the guidance to `.github/copilot-instructions.md`. Both `.vscode/mcp.json` and `.github/hooks/token-goat.json` then hold absolute paths to node and token-goat on your machine, so do not commit them or the `token-goat-shim.js` next to them: list them in `.git/info/exclude` (just for you) or `.gitignore`. Install prints this reminder.
...
Both `.mcp.json` files are also read by Claude Code, under a different key (`mcpServers`), and Claude Code stops with an error on a `.mcp.json` that has no `mcpServers` key. So token-goat writes its entry under `servers` and, when the file has no `mcpServers` key yet, adds an empty one. It registers nothing for Claude Code and leaves your own `mcpServers` entries as they were. A timestamped `.bak` is written before any change to an existing `.mcp.json`. Visual Studio also reads `.vscode/mcp.json`, so if `token-goat install --vscode -p` put token-goat there too, Visual Studio lists the server twice. The install prints a note when that happens, and `token-goat doctor` warns about it; keep one of the two. If `--vscode -p` or `--copilot --local` already put a token-goat block in `.github/copilot-instructions.md`, the Visual Studio block shrinks to a short addendum rather than repeating it, and grows back to the full text when that other block is removed.
