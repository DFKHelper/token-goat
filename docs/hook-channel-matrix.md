...
| event / channel | claudecode | copilot_cli | vscode | codex | grok | gemini | qwen | kimi | opencode | openclaw | pi |
|---|---|---|---|---|---|---|---|---|---|---|---|
| pre deny | M (d) | M (w)¹ | M (w)²⁸ | ? (doc, BE-06) | M (doc)² | ? (doc)³ | ? (doc)⁴ | M (w)⁵ | M (w)⁶ | M (doc) | M (w) |
| pre context (hints) | M (c) | M (w)⁷ | M (w)²⁸ | ? (u, BE-06) | D (b)⁸ | ? (u) | ? (doc) | ? (u)⁹ | D (b)¹⁰ | D (b)¹⁰ | D (b)¹⁰ |
...
| pre_compact context (manifest) | M (c)²⁰ | D (w)²¹ | X³¹ | ? (u, BE-06) | D (doc)² | ? (u) | ? (doc) | ? (u) | M (w)²² | D (b)²³ | M (w)²⁴ |
| session_start context | M (d) | M (c)²⁵ | M (w)²⁸ | X | X | X | X | ? (u) | X | X | X |
| user_prompt_submit context | M (d) | M (c)²⁶ | ? (u) | ? (u, BE-06) | D (doc)² | X | ? (doc) | M (w)²⁷ | X | X | X |
...
7. Was **D** until loop 56, on the claim that Copilot's preToolUse output schema has no `additionalContext` field, and the claim was simply false. `PreToolUseHookOutput` in the shipping `copilot-sdk/types.d.ts` declares `additionalContext?: string` in 1.0.80, 1.0.82, 1.0.86, 1.0.87 and 1.0.88 alike — including the version every other Copilot verdict in this table was read from — and the 1.0.88 native runtime (`prebuilds/win32-x64/runtime.node`) carries the key in the preToolUse response key set beside the three already known to work: `permissionDecision`, `permissionDecisionReason`, `modifiedArgs`, `additionalContext`, in that order, immediately before the `preToolUse hook timed out; allowing the tool call to proceed` message. The same binary carries `Ignoring additionalContext returned by a ... hook ... because aggregating it would exceed the ... byte limit`, which names the field generically and is the aggregation 1.0.88's release notes describe. `translate()` now forwards the hint, alongside `modifiedArgs` rather than instead of it. The one payload deliberately still withheld is the image-shrink data URL — see footnote 11.
...
16. BE-12 resolved loop 55, from each API's own source, and the two harnesses split. **pi: the result IS mutable, and the rewrite now ships.** @earendil-works/pi-coding-agent@0.84.3 (npm tarball) declares `ToolResultEvent` as "Fired after a tool executes. Can modify result." with a `ToolResultEventResult` of `{content, details, isError, usage}` (`dist/core/extensions/types.d.ts`), and the shipped runtime applies it: `emitToolResult` copies a handler's `content` onto the event (`dist/core/extensions/runner.js`) and `afterToolCall` returns `hookResult?.content ?? result.content` as the tool result (`dist/core/agent-session.js`). So every post_tool_use rewriteOutput was computed, cached, stat-recorded and then discarded on pi. The bridge now returns the rewrite, replacing only the text blocks and preserving image blocks in place, because `emitToolResult` overwrites `content` wholesale; `details`/`isError`/`usage` are deliberately not returned, since the runtime only overwrites fields a handler actually supplies. **OpenClaw: `after_tool_call` genuinely cannot.** openclaw/openclaw `docs/plugins/hooks.md` classifies it as an Observe hook and states the contract outright: "Handlers run concurrently; return values are ignored." That is a proven negative, not a gap. A different OpenClaw hook, `tool_result_persist` ("Sync modify", returns `{message}` to replace a tool result, and its event carries `toolName`/`toolCallId` per `src/agents/session-tool-result-guard-wrapper.ts`), is a real rewrite surface and the docs call a persisted result's `content` model-visible. It is NOT wired, for two reasons recorded rather than coded around: it is synchronous-only ("returned promises are ignored with a warning"), which would force a full subprocess spawn per tool result and give up the in-process relay, and whether the rewrite reaches the CURRENT turn's model input or only later replay is unproven. Filed BE-16.
...
- **Loop 49's unrestricted-spawn advisory** rode post_tool_use additionalContext, dropped on
  ...
  BE-06).
- **Every pre-tool hint was dropped on Copilot CLI** (footnote 7) — read-dedup nudges, the
  surgical-read advisories, the write-path notices — on a schema claim the vendor's own type
  definitions contradict, in the same version the rest of the Copilot column was read from. The
  drop was recorded honestly and was still a drop. Fixed loop 56. The general lesson is the one
  footnote 17 already half-states: this table's rows are per channel per event, and "the harness
  has no channel here" has to be read off that event's own schema, not inferred from a sibling
  event that really does lack it.
- **Copilot's hook processing left `app.js` in 1.0.82.** Every bundle offset cited for Copilot in
  this document and in `src/bridges/copilot_cli.ts` is a 1.0.80 offset, and in 1.0.82 the bundle
  shed roughly 1.7 MB and with it `additionalContext`, `preToolUse` and all but four
  `textResultForLlm` occurrences. The channels did not die: they moved into
  `prebuilds/<platform>/runtime.node`, which carries every one of those strings in 1.0.88. A
  future loop grepping `app.js` and finding nothing must not read that as a channel closing.
...
- **pi dropped every post_tool_use rewriteOutput** (footnote 16). Fixed loop 55, after pi's own
  package proved the tool result is mutable there.
- Grok (deny-only), Kimi (no rewrite channels, post fire-and-forget), OpenClaw post-response
  drops: harness-design limitations, recorded here and in the bridges' own comments, deliberately
  not coded around. OpenClaw's `after_tool_call` is a proven negative as of loop 55.
- Every `?` cell is an unverified finding: Codex/Gemini/Grok/Kimi wire maps are BE-06; Qwen
  response honoring is BE-13; the specific residuals are BE-10/BE-11/BE-15/BE-16.
