...
- **`families.ts`** — filter *family factories*: shared compression skeletons that several per-tool filters configure instead of reimplementing. `makeNodeTestRunnerFilter(cfg)` is the first — one line-classifier loop (file PASS/FAIL headers, per-test ticks, collapsible console/stdout blocks, always-kept summary, optional Jest `--verbose` duplicate-`Failures:` drop), parameterised by a few regexes + nouns so the next runner is a config object, not another loop. All loop state is per-call, so a single shared filter instance is concurrency-safe.
