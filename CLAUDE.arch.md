...
- **Hook wiring (`hooks_bash.ts`)** — `preBashHandler` makes the rewrite decision: for a recognized single command with no cached prior run it returns a `rewriteInput` HookOutput, which [`hook_registry.ts::serializeOutput`](src/hook_registry.ts) emits as the `PreToolUse` wire shape `{hookSpecificOutput:{hookEventName:"PreToolUse",permissionDecision:"allow",updatedInput}}`. `updatedInput` replaces the *entire* `tool_input`, so the hook spreads `{...event.toolInput, command: wrapped}` to keep `description`/`timeout`. Filter selection runs on the cd-*stripped* command (`detectFromCommand`, with the `generic` filter as the build/test-command fallback while specific filters are still being ported), but the wrapper quotes the *full* original command including any `cd … &&` prefix so it still runs in the right cwd. A cached prior run wins over a fresh rewrite (recall is cheaper than re-running and re-compressing). Because the rewrite changes the command the post-hook then sees, `postBashHandler` unwraps `token-goat compress … -c <orig>` back to `<orig>` before keying its output cache — so recall stays keyed on the original command and round-trips across runs. Compounds, pipes, and redirects are never wrapped (`isCompressibleSingleCommand` mirrors `detectFromCommand`'s single-command rule). Honored opt-outs: `TOKEN_GOAT_BASH_COMPRESS=0` and per-filter `[bash_compress] disabled_filters`.
