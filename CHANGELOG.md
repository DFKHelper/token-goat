...
- **Pre-Grep matcher + pre-Bash matcher in install.** ``PreToolUse`` now fires on ``Read|Grep|Bash`` (was ``Read``-only) so the new Grep dedup hint actually runs.  Pre-existing Bash dedup hint logic in ``hooks_read`` now reaches the wire too — under the old matcher it only fired under Codex where the matcher was already permissive.
...
- **New stats bucket `bash` (orange) and diff-hint accounting.** `token-goat stats` now attributes `diff_hint` / `diff_hint_overhead` to the `hint` bucket and `bash_dedup_hint*` / `bash_output_cached` to a new visible `bash` bucket, so the new mechanisms appear in the by-source panel instead of falling into the `other` catch-all.
